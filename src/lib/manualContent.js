export const MANUALS = {
  commissioner: {
    ...
    chapters: [
      ...
      { title: "2. Set rules and teams", summary: "Open Setup and work through the commissioner workspace before anyone drafts.", steps: [
        ...
        "To move a current league setup or complete roster from a spreadsheet, download the League Import XLSX or CSV template. Keep only supported Team, Manager, Pokémon, and Price data, upload one file, repair every preview error, and confirm only after the parsed revision matches the source. Manager labels never claim accounts, and a complete-roster confirmation requires the exact league name.",
      ...
      { title: "5. Operate the regular season", summary: "DraftCenter connects the schedule, reported results, standings, transactions, and league communication.", steps: [
        ...
        "For an eligible scheduled regular-season matchup, a participating manager may analyze one to five exact public Pokémon Showdown replay URLs, map each Showdown player to the scheduled teams, review the normal result editor, and choose Save. Analysis never writes a result automatically, and manual score changes clear the replay confirmation.",
  ...
  manager: {
    ...
    description: "A practical DraftCenter manual for accepting a league invite, claiming a team, drafting, reporting manual or Showdown-supported results, and managing transactions.",
    ...
    chapters: [
      ...
      { title: "4. Prepare, play, and report matches", summary: "Use Schedule as the source of truth for opponents, deadlines, and results while preparing ahead in My Team.", steps: ["In My Team, open Private Opponent Prep and choose any matchup already present on the schedule. Past, current, and future round-robin opponents can open in the private Team Lab overlay without changing the league's official current week. Swiss opponents appear only after each new round is paired.", "Use the complete official roster lists to choose the six Pokémon you want to analyze for each side. Save the league roster as a private My Teams planning copy before saving matchup notes; this never changes either official league roster.", "Coordinate the battle with your opponent, build only from your current legal roster, and play under the league rules.", "Either participating manager can report the result and eligible MVP information. Check the saved result and standings after submitting; a Swiss bye already counts as a match win and needs no report.", "For an eligible scheduled regular-season matchup, paste one to five exact public Pokémon Showdown replay URLs, analyze them, and deliberately map each Showdown player to the scheduled teams. Review the proposed score and supported facts before pressing Save. Use manual entry when a replay is unavailable or unsupported.", "Share a live Twitch or YouTube match from League Home if desired. Choose the audience carefully; private leagues never appear in public discovery."]},
