export const MANUALS = {
  commissioner: {
    ...
    chapters: [
      ...
      { title: "2. Set rules and teams", summary: "Open Setup and work through the commissioner workspace before anyone drafts.", steps: [
        ...
        "Set transaction limits, regular-season format, standings tiebreakers, playoff size, and the league clock for match and claim deadlines. Swiss supports one shared 4–16 team table; round robin remains available for divisions and larger leagues.",
        "Save a recovery backup after major setup milestones. Use the spreadsheet export when you need a readable offline record.",
        "Commissioner Tools also shows Automatic Recovery History. Review the team, roster, result, season, and draft summary before restoring; type the exact league name to confirm.",
      ]},
      ...
      { title: "5. Operate the regular season", summary: "DraftCenter connects the schedule, reported results, standings, transactions, and league communication.", steps: [
        "For round robin, generate or edit the schedule. For Swiss, pair Round 1 after the draft, report every current result, then pair the next unlocked round. Either participating manager may report their matchup.",
        "Swiss ranks by wins, OMWP, GWP, OGWP, then initial team order. A bye counts as a match win. Correct an earlier score before later play starts; a competitive correction may remove still-empty future pairings so they can be rebuilt safely.",
  ...
  manager: {
    ...
    chapters: [
      ...
      { title: "4. Play and report matches", summary: "Use Schedule as the source of truth for opponents, deadlines, and results.", steps: ["Coordinate the battle with your opponent, build only from your current legal roster, and play under the league rules. In Swiss, each new opponent appears only after every current-round result is complete.", "Either participating manager can report the result and eligible MVP information. Check the saved result and standings after submitting; a Swiss bye already counts as a match win and needs no report.", "Share a live Twitch or YouTube match from League Home if desired. Choose the audience carefully; private leagues never appear in public discovery."]},
