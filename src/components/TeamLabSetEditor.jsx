  ...
  return <section className="team-lab-set-editor" aria-labelledby="team-lab-set-editor-title">
    ...
    {!rosterNames.length ? <p className="team-lab-matchup-empty">Add Pokémon to the roster before building sets.</p> : <>
      ...
      {selected && <div className="team-lab-set-form">
        ...
        <div className="team-lab-set-fields"><label>Ability<TeamLabBattleAutocomplete kind="ability" pokemonName={selected.name} regulationId={formatId} preferred={[selected.ability]} maxLength={100} value={selected.ability} onChange={(event) => updateSelected({ ability: event.target.value })} placeholder="Rough Skin"/></label><label>Held item<TeamLabBattleAutocomplete kind="item" pokemonName={selected.name} regulationId={formatId} preferred={[selected.item]} maxLength={100} value={selected.item} onChange={(event) => updateSelected({ item: event.target.value })} placeholder="Choice Scarf"/></label><label>Nature<input maxLength={30} value={selected.nature} onChange={(event) => updateSelected({ nature: event.target.value })} placeholder="Jolly"/></label>{usesTera && <label>Tera type<select value={selected.tera_type} onChange={(event) => updateSelected({ tera_type: event.target.value })}><option value="">Unspecified</option>{TYPES.map((type) => <option key={type}>{type}</option>)}</select></label>}</div>
        ...
        <fieldset className="team-lab-set-moves"><legend>Moves</legend><div>{Array.from({ length: 4 }, (_, index) => <label key={index}>Move {index + 1}<TeamLabBattleAutocomplete kind="move" pokemonName={selected.name} regulationId={formatId} preferred={selected.moves} maxLength={100} value={selected.moves[index] || ""} onChange={(event) => { const moves = [...selected.moves]; moves[index] = event.target.value; updateSelected({ moves }); }} placeholder="Move name"/></label>)}</div></fieldset>
