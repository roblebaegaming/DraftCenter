...
function PublicLeagueDetails({ league, membership, busy, onClose, onOpen, onJoin }) {
  ...
  const action = membership ? (membership.role === "commissioner" ? "Manage league" : "Open league setup") : "View setup and choose a team";
...
return (
  <main className="hub-shell">
    ...
    {message && <p className="hub-message">{message}</p>}
    {pendingInvite && <section className="hub-card invite-confirm"><span className="eyebrow">LEAGUE INVITATION</span><h2>{pendingInvite.is_spectator ? "Watch this league?" : "Open this league?"}</h2><p><strong>{pendingInvite.league_name}</strong>{pendingInvite.season_label ? ` - ${pendingInvite.season_label}` : ""}</p><p className="muted">{pendingInvite.is_spectator ? "You will have spectator access only. You can view and scout, but cannot claim a team or change league data." : "Review the saved setup first. A competitive spot is only taken after you choose and claim a team."}</p><div className="flex gap-2 flex-wrap"><button className="primary-button" disabled={inviteBusy} onClick={acceptPendingInvite}>{inviteBusy ? "Opening..." : "Open league setup"}</button><button className="quiet-button" disabled={inviteBusy} onClick={dismissInvite}>Not now</button></div></section>}
