...
import { buildWeeklyLeagueAgenda } from "../lib/leagueNextActions";
...
function PollCommentThread({ comment, replies, onReply, onUpvote, onProfile }) {
  const author = (row) => <CoachProfileButton compact username={row.username} displayName={row.display_name} avatarUrl={row.avatar_url} onOpen={onProfile}/>;
...
export function PollOfTheDay({ supabase }) {
  ...
  return <section className="hub-card poll-card">{profileIdentity&&<PublicCoachProfile identity={profileIdentity} onClose={()=>setProfileIdentity("")}/>}
...
function PublicLeagueDetails({ league, membership, busy, onClose, onOpen, onJoin }) {
  ...
  return <div className="modal-backdrop"><section className="tools-modal"><button className="modal-close" onClick={onClose}>x</button><span className="eyebrow">{watchOnly ? "PUBLIC TO WATCH" : "OPEN TO JOIN"}</span>{league.image_url && <img className="league-cover" src={league.image_url} alt={`${league.name} cover`} />}{league.is_practice && <span className="practice-badge">Practice league</span>}<h2>{league.name}</h2><p className="muted">{league.season_label || "New season"}</p><p><strong>Draft:</strong> {formatDraftStart(league.draft_starts_at)}</p><p><strong>Teams claimed:</strong> {league.filled_spots || 0}{league.total_spots ? ` / ${league.total_spots}` : ""}</p><PublicDraftDetails league={league} /><p>{league.description || "The commissioner has not added a league description yet."}</p><button className="primary-button" disabled={busy} onClick={() => membership ? onOpen({ ...league, role: membership.role }) : watchOnly ? window.location.assign(`/league/${league.slug}`) : onJoin(league)}>{action}</button></section></div>;
}
...
export default function LeagueHub({ user, profile, onOpenLeague }) {
  ...
  async function acceptPendingInvite() {
    ...
    const [{ data: league }, { data: snapshot }] = await Promise.all([
      supabase.from("leagues").select("id, name, slug, description, image_url, season_label, status, draft_starts_at, league_visibility, draft_start_visibility, is_practice, practice_expires_at").eq("id", leagueId).single(),
  ...
  const weeklyAgenda = buildWeeklyLeagueAgenda(activeLeagues, leagueStates, { userId: user.id, identity: profile?.display_name || profile?.username });
  function openCommissionerSetup(practice = false) {
    setIsPractice(practice);
    if (practice) setVisibility("private");
    trackActivationEvent(practice ? "practice_path_started" : "commissioner_path_started", { properties: { source: "dashboard", practice: practice ? "yes" : "no" } });
    const details = document.getElementById("run-a-league");
...
return (
  <main className="hub-shell">
    ...
    <nav className="hub-primary-paths" aria-label="Choose your DraftCenter path"><button type="button" className="primary-button" onClick={() => openCommissionerSetup(false)}>Run a league</button><a className="secondary-button" href="#public-leagues">Join a league</a><a className="quiet-button" href="/team-lab">Prepare for a match</a></nav>
    ...
    {message && <p className="hub-message">{message}</p>}
    {pendingInvite && <section className="hub-card invite-confirm"><span className="eyebrow">LEAGUE INVITATION</span><h2>{pendingInvite.is_spectator ? "Watch this league?" : pendingInvite.role === "co_commissioner" ? "Become a League Manager?" : "Join this league?"}</h2><p><strong>{pendingInvite.league_name}</strong>{pendingInvite.season_label ? ` - ${pendingInvite.season_label}` : ""}</p><p className="muted">{pendingInvite.is_spectator ? "You will have spectator access only. You can view and scout, but cannot claim a team or change league data." : pendingInvite.role === "co_commissioner" ? "Accepting gives you League Manager access—DraftCenter’s co-commissioner permission level—for this league only. You can manage its settings, scheduling, results, and commissioner tools; linked sibling pods remain view-only unless you are assigned there separately." : "Accept the invitation, then choose one of the league’s currently open teams. A competitive spot is only taken after you claim it."}</p><div className="flex gap-2 flex-wrap"><button className="primary-button" disabled={inviteBusy} onClick={acceptPendingInvite}>{inviteBusy ? "Accepting..." : pendingInvite.role === "co_commissioner" ? "Accept League Manager role" : pendingInvite.is_spectator ? "Watch league" : "Accept & choose a team"}</button><button className="quiet-button" disabled={inviteBusy} onClick={dismissInvite}>Not now</button></div></section>}
    ...
    {commissionerIntent && leagues.length === 0 && <section className="hub-card commissioner-intent-card"><div><span className="eyebrow">YOUR FIRST LEAGUE</span><h2>{commissionerIntent === "practice" ? "Start safely with a practice season" : "Your commissioner setup is ready"}</h2><p>{commissionerIntent === "practice" ? "Practice leagues stay private and out of career statistics. You can run the complete flow before inviting anyone." : "Name the season now, then use the guided checklist to reach a scheduled draft."}</p></div><button type="button" className="primary-button" onClick={() => openCommissionerSetup(commissionerIntent === "practice")}>Continue setup</button></section>}
    {weeklyAgenda.length > 0 && <section className="hub-card weekly-agenda"><div className="section-heading"><div><span className="eyebrow">THIS WEEK</span><h2>What happens next?</h2></div><span className="weekly-health">{weeklyAgenda.length} active league{weeklyAgenda.length === 1 ? "" : "s"}</span></div><p className="muted">Your next draft, match, or commissioner action—without exposing private preparation.</p><div className="weekly-agenda-list">{weeklyAgenda.slice(0, 5).map((item) => <a key={item.leagueId} href={item.href}><span>{item.leagueName}</span><strong>{item.title}</strong><small>{item.detail}{item.updatedAt ? ` · Updated ${new Date(item.updatedAt).toLocaleDateString()}` : ""}</small></a>)}</div></section>}
    ...
    <section className="hub-card public-card" id="public-leagues"><div className="section-heading"><div><span className="eyebrow">DISCOVER</span><h2>Public leagues</h2></div><span className="muted">Join a team or follow a season.</span></div><div className="flex gap-2 mb-5"><button className={publicTab === "join" ? "primary-button" : "quiet-button"} onClick={() => setPublicTab("join")}>Open to join</button><button className={publicTab === "watch" ? "primary-button" : "quiet-button"} onClick={() => setPublicTab("watch")}>Open to watch</button></div>{!loading && publicLeagues.filter((league) => league.league_visibility === (publicTab === "join" ? "open" : "watch")).length === 0 && <p className="muted">{publicTab === "join" ? "No leagues are open for new managers yet." : "No watchable leagues have been published yet."}</p>}<div className="public-grid">{publicLeagues.filter((league) => league.league_visibility === (publicTab === "join" ? "open" : "watch")).map((league) => { const membership = membershipFor(league); const watchOnly = league.league_visibility === "watch"; return <article key={league.id} className="public-league">{league.image_url && <img className="public-league-image" src={league.image_url} alt="" />}<button className="public-league-title" onClick={() => setPublicDetails(league)}><strong>{league.name}</strong><span>View league details</span></button><p>{league.description || league.season_label || "Public league"}</p><small className="muted">{formatDraftStart(league.draft_starts_at)} · {league.filled_spots || 0}{league.total_spots ? ` / ${league.total_spots}` : ""} teams claimed</small><PublicDraftDetails league={league} compact />{league.is_practice && <span className="practice-badge">Practice</span>}<button className="secondary-button" disabled={busy} onClick={() => membership ? onOpenLeague({ ...league, role: membership.role }) : watchOnly ? window.location.assign(`/league/${league.slug}`) : joinPublicLeague(league)}>{membership ? (membership.role === "commissioner" ? "Manage league" : "Open your league") : watchOnly ? "Watch league" : "Join as a manager"}</button></article>; })}</div></section>
    <details id="run-a-league" className="hub-card create-card" defaultOpen={Boolean(commissionerIntent && leagues.length === 0)}><summary><span><span className="eyebrow">RUN A LEAGUE</span><strong>Start a new season</strong><small>Create the league now, then follow one guided setup checklist.</small></span><b>Open setup</b></summary><div className="create-card-body"><p className="muted">Choose a starting path. Format, team count, Pokémon pool, prices, and detailed rules come next inside the league.</p><div className="league-starting-paths"><button type="button" className={!isPractice && visibility === "private" ? "selected" : ""} onClick={() => { setIsPractice(false); setVisibility("private"); }}>Private season<small>Recommended for an established group</small></button><button type="button" className={isPractice ? "selected" : ""} onClick={() => { setIsPractice(true); setVisibility("private"); }}>Practice walkthrough<small>Safe, private, and excluded from career stats</small></button><button type="button" className={!isPractice && visibility === "open" ? "selected" : ""} onClick={() => { setIsPractice(false); setVisibility("open"); }}>Open community league<small>Visible to managers looking for a team</small></button></div><form onSubmit={createLeague} className="form-stack"><label>League name<input required minLength={2} value={name} onChange={(e) => setName(e.target.value)} placeholder="Kanto Cup" /></label><label>Season label<input value={season} onChange={(e) => setSeason(e.target.value)} placeholder="Season 1" /></label><label>Draft start date and time (optional)<input type="datetime-local" value={draftStartsAt} onChange={(e) => setDraftStartsAt(e.target.value)} /><small>You can leave this blank and schedule it from the launch checklist.</small></label><label>Short public description<textarea rows={3} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Who is this league for, and what format will it play?" /></label><label>League image URL (optional)<input type="url" value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} placeholder="https://..." /></label><label>Who can access it before the draft?<select value={visibility} onChange={(e) => setVisibility(e.target.value)}><option value="private">Private - invite link only</option><option value="watch">Public to watch - people can view setup</option><option value="open">Open to join - shown in Discover</option></select></label><label>When the draft starts<select value={draftStartVisibility} onChange={(e)=>setDraftStartVisibility(e.target.value)}><option value="default">Default - close joining and keep the season public</option><option value="private">Make the league private</option><option value="watch">Make the season public to watch</option></select></label><label className="check-label"><input type="checkbox" checked={isPractice} onChange={(e) => setIsPractice(e.target.checked)} /> <span>Practice league - kept out of career stats</span></label><button className="primary-button" disabled={busy}>{busy ? "Working..." : isPractice ? "Create practice league" : "Create league & open checklist"}</button></form></div></details>
    ...
    <section className="hub-home-tools" aria-labelledby="hub-home-tools-title"><div><span className="eyebrow">TEAM-BUILDING TOOL</span><h2 id="hub-home-tools-title">Team Lab</h2><p>Build a six-Pokémon team, import a PokéPaste, connect saved teams, write private notes, plan opponent matchups, and check coverage, legality, and common meta archetypes.</p></div><a className="secondary-button inline-link-button" href="/team-lab">Open Team Lab</a></section>
  </main>
);
}
