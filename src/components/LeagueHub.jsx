...
function PollOfTheDay({ supabase }) {
  ...
  return <section className="hub-card poll-card">
    ...
    <label className="poll-history-picker poll-history-footer">Browse previous polls<select value={poll?.id || ""} onChange={(event) => setPoll(history.find((item) => item.id === event.target.value) || poll)}>{history.map((item) => <option key={item.id} value={item.id}>{new Date(`${item.poll_date}T12:00:00`).toLocaleDateString()} — {item.question}</option>)}</select></label>
  </section>;
}
