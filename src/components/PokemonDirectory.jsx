...
const LEGENDS_ZA_SPECIES_BY_POKEDEX = Object.freeze(Object.fromEntries(LEGENDS_ZA_POKEDEX.pokedexes.map(({ key }) => [
  key,
  new Set(LEGENDS_ZA_POKEDEX.entries
    .filter((entry) => entry.pokedex_key === key)
...
function matchesGamePokedex(name, filterKey) {
  ...
  return filter.pokedexKeys.some((pokedexKey) => LEGENDS_ZA_SPECIES_BY_POKEDEX[pokedexKey]?.has(speciesKey))
    && pokemonShowdownProfileKeys(name).some((profileKey) => LEGENDS_ZA_AVAILABLE_PROFILE_KEYS.has(profileKey));
}
