...
const FORM_REFERENCE_OVERRIDES = {
  "mega-absol": { apiName: "absol-mega-z", speciesName: "absol", dexName: "absol" },
  "mega-garchomp": { apiName: "garchomp-mega-z", speciesName: "garchomp", dexName: "garchomp" },
  "mega-lucario": { apiName: "lucario-mega-z", speciesName: "lucario", dexName: "lucario" },
...
function pokemonDirectoryTraits(name) {
  const reference = pokemonReference(name);
  return pokemonSpeciesTraitsForProfile(reference.apiName) || pokemonSpeciesTraitsForProfile(reference.speciesName);
}

function validTraitParam(options, value) {
  return options.some((option) => option.id === value) ? value : "";
}

const LEGENDS_ZA_AVAILABLE_PROFILE_KEYS = new Set(LEGENDS_ZA_POKEDEX.available_profile_keys);
const LEGENDS_ZA_SPECIES_BY_POKEDEX = Object.freeze(Object.fromEntries(LEGENDS_ZA_POKEDEX.pokedexes.map(({ key }) => [
  key,
  new Set(LEGENDS_ZA_POKEDEX.entries.filter((entry) => entry.pokedex_key === key).map((entry) => pokemonBaseSpeciesKey(entry.pokemon_key))),
])));

function validGameParam(value) {
  return pokedexGameFilterByKey(value) ? value : "";
}

function matchesGamePokedex(name, filterKey) {
  if (!filterKey) return true;
  const filter = pokedexGameFilterByKey(filterKey);
  if (!filter || filter.gameKey !== "legends-za") return true;
  const speciesKey = pokemonBaseSpeciesKey(name);
  const speciesMatches = filter.pokedexKeys.some((pokedexKey) => LEGENDS_ZA_SPECIES_BY_POKEDEX[pokedexKey]?.has(speciesKey));
  return speciesMatches && pokemonShowdownProfileKeys(name).some((profileKey) => LEGENDS_ZA_AVAILABLE_PROFILE_KEYS.has(profileKey));
}
...
function PokemonDailyThreeProfile({ profile }) {
  if (!profile) return <p className="muted">Loading Daily Games highlights…</p>;
...
function PokemonDraftProfile({ profile }) {
  ...
  return <div className="pokemon-draft-profile">
    <div className="pokemon-draft-metrics">
      <article><strong>{hasDrafts ? `${profile.draft_rate || 0}%` : "—"}</strong><span>Draft rate</span><small>{profile.drafted_in || 0} of {profile.eligible_drafts || 0} completed drafts</small></article>
    ...
    {profile.adp_by_format?.length ? <ol className="pokemon-partner-list">{profile.adp_by_format.map((item) => <li key={item.regulation_id}><strong>{displayName(item.regulation_id || "custom")}</strong><span>{item.average_pick != null ? `ADP #${item.average_pick}` : "Not drafted"} · {item.drafted_in || 0} of {item.eligible_drafts || 0} eligible drafts</span></li>)}</ol> : <p className="muted">Format-specific ADP will appear after this Pokémon is included in a completed snake draft pool.</p>}
    <h4>Most common teammates</h4>
    {profile.partners?.length ? <ol className="pokemon-partner-list">{profile.partners.slice(0, 5).map((item) => <li key={item.pokemon}><strong>{item.pokemon}</strong><span>{item.teams} team{item.teams === 1 ? "" : "s"}</span></li>)}</ol> : <p className="muted">Partner data will appear as league rosters fill.</p>}
  ...
  return <main className="pokemon-directory pokemon-directory-wide">
    ...
    <section className="pokemon-directory-layout">
      <aside className="pokemon-search-card pokemon-browser-card">
        <div className="pokemon-browser-controls"><form className="pokemon-main-search" onSubmit={props.searchPokemon}><label>Search Pokemon or Dex #<input value={props.query} onChange={(event) => props.handlePokemonQueryChange(event.target.value)} placeholder="Pikachu, Garchomp, 681..." autoFocus /></label><button className="quiet-button" type="submit">Search</button></form><label>Type<select value={props.type} onChange={(event) => props.setType(event.target.value)}><option value="">All types</option>{TYPES.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label>Generation<select value={props.generation} onChange={(event) => props.setGeneration(event.target.value)}><option value="">All generations</option>{[1,2,3,4,5,6,7,8,9].map((item) => <option key={item} value={item}>Gen {item}</option>)}</select></label><label>Game Pokédex<select value={props.game} onChange={(event) => props.setGame(event.target.value)}><option value="">All games</option>{POKEDEX_GAME_FILTERS.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}</select></label><div className="pokemon-list-controls"><label>Show<select value={props.resultLimit} onChange={(event) => props.setResultLimit(event.target.value === "all" ? "all" : Number(event.target.value))}><option value={100}>100</option><option value={200}>200</option><option value="all">All</option></select></label><div className="pokemon-community-sort"><span>Sort by community wins</span><div><button type="button" title="Poll wins" aria-label="Sort by Poll wins" className={props.sortBy === "poll_wins" ? "active" : ""} onClick={() => props.toggleSort("poll_wins")}><b>🏆</b><small>Polls</small></button><button type="button" title="Bracket championships" aria-label="Sort by Bracket championships" className={props.sortBy === "bracket_championships" ? "active" : ""} onClick={() => props.toggleSort("bracket_championships")}><b>👑</b><small>Brackets</small></button><button type="button" title="Most-popular quiz answers" aria-label="Sort by Most-popular quiz answers" className={props.sortBy === "quiz_popular_finishes" ? "active" : ""} onClick={() => props.toggleSort("quiz_popular_finishes")}><b>⭐</b><small>Quizzes</small></button></div></div></div><div className="pokemon-size-sort"><span>Sort by size</span><div><button type="button" className={props.sortBy === "height" ? "active" : ""} onClick={() => props.toggleSort("height")}>Height {props.sortBy === "height" ? (props.sortDirection === "asc" ? "↑" : "↓") : "↕"}</button><button type="button" className={props.sortBy === "weight" ? "active" : ""} onClick={() => props.toggleSort("weight")}>Weight {props.sortBy === "weight" ? (props.sortDirection === "asc" ? "↑" : "↓") : "↕"}</button></div><small>{props.dimensionLoadedCount < props.filteredCount ? `Loading measurements ${props.dimensionLoadedCount}/${props.filteredCount}` : "All measurements ready"}</small></div></div>
        <fieldset className="pokemon-trait-filters">
          <legend>Pokédex traits</legend>
          <div className="pokemon-trait-filter-grid">
            <label className={`pokemon-trait-filter pokemon-trait-color ${props.color ? "is-active" : ""}`}><span>Color</span><select value={props.color} onChange={(event) => props.setColor(event.target.value)}><option value="">All colors</option>{POKEMON_COLOR_OPTIONS.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
            <label className={`pokemon-trait-filter pokemon-trait-egg-group ${props.eggGroup ? "is-active" : ""}`}><span>Egg Group</span><select value={props.eggGroup} onChange={(event) => props.setEggGroup(event.target.value)}><option value="">All Egg Groups</option>{POKEMON_EGG_GROUP_OPTIONS.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
            <label className={`pokemon-trait-filter pokemon-trait-shape ${props.shape ? "is-active" : ""}`}><span>Shape</span><select value={props.shape} onChange={(event) => props.setShape(event.target.value)}><option value="">All shapes</option>{POKEMON_SHAPE_OPTIONS.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
          </div>
        </fieldset>
      ...
      <section className="pokemon-detail-card pokemon-detail-wide">
        ...
        {props.details && props.species && <>
          ...
          <div className="pokemon-detail-grid"><section><h3>All base stats</h3><div className="pokemon-stats">{props.details.stats.map((stat) => <div key={stat.stat.name}><span>{displayName(stat.stat.name).replace("special", "Sp.")}</span><strong>{stat.base_stat}</strong></div>)}<div className="pokemon-bst-total"><span>BST</span><strong>{props.details.stats.reduce((sum, stat) => sum + stat.base_stat, 0)}</strong></div></div></section><section><h3>Abilities</h3><div className="pokemon-tags">{props.details.abilities.map(({ ability, is_hidden }) => <span key={ability.name}>{displayName(ability.name)}{is_hidden ? " (hidden)" : ""}</span>)}</div><h3>DraftCenter poll podiums</h3><PokemonPollPlacements placements={props.pollPlacements} /><h3>Daily Games highlights</h3><PokemonDailyThreeProfile profile={props.dailyThreeProfile} /></section></div>
          <section className="pokemon-community-stats"><h3>DraftCenter community statistics</h3><p className="muted">Anonymous aggregates from all DraftCenter leagues, including private leagues. Percentages always include their current sample size.</p><PokemonDraftProfile profile={props.draftProfile} /></section>
...
function PokemonDirectoryContent() {
  ...
  useEffect(() => { try { const cached = JSON.parse(localStorage.getItem("draftcenter:pokemon-dimensions:v1") || "{}"); if (cached && typeof cached === "object") setStatLookup((current) => ({ ...current, ...cached })); } catch {} }, []);
  useEffect(() => { createClient().rpc("get_pokemon_community_ranking_totals").then(({ data }) => { const lookup = {}; (data || []).forEach((row) => { lookup[String(row.pokemon_key || "").toLowerCase()] = row; }); setCommunityRankingLookup(lookup); }); }, []);
  ...
  const filteredPokemon = useMemo(() => POKEMON_DIRECTORY.filter((pokemon) => {
    const term = query.trim().toLowerCase().replace(/^#/, "");
    const reference = pokemonReference(pokemon.name);
    const traits = pokemonDirectoryTraits(pokemon.name);
    const dex = dexNumbers[reference.dexName] || dexNumbers[reference.apiName];
    const numericMatch = /^\d+$/.test(term) && (String(dex || "").includes(term) || reference.dexName === dexSearchName);
    const matchName = !term || pokemon.name.toLowerCase().includes(term) || numericMatch;
    const matchType = !type || pokemon.t1 === type || pokemon.t2 === type;
    const matchGen = !generation || String(pokemon.gen) === generation;
    const matchGame = matchesGamePokedex(pokemon.name, game);
    const matchColor = !color || traits?.color === color;
    const matchEggGroup = !eggGroup || traits?.egg_groups?.includes(eggGroup);
    const matchShape = !shape || traits?.shape === shape;
    const matchAbility = !abilityMatches || abilityMatches.has(slug(pokemon.name));
    return matchName && matchType && matchGen && matchGame && matchColor && matchEggGroup && matchShape && matchAbility;
  }), [query, type, generation, game, color, eggGroup, shape, abilityMatches, dexNumbers, dexSearchName]);
  useEffect(() => { let cancelled = false; const targets = filteredPokemon.filter((pokemon) => { const cached = statLookup[pokemonReference(pokemon.name).apiName]; return cached === undefined || cached.height === undefined || cached.weight === undefined; }).slice(0, 400); if (!targets.length) return undefined; async function loadOne(pokemon) { const reference = pokemonReference(pokemon.name); const key = reference.apiName; for (let attempt = 0; attempt < 3; attempt += 1) { try { const response = await fetchPokemonForm(reference); if (response.ok) { const data = await response.json(); const stats = Object.fromEntries((data.stats || []).map((entry) => [entry.stat.name, entry.base_stat])); return [key, { hp: stats.hp, attack: stats.attack, defense: stats.defense, "special-attack": stats["special-attack"], "special-defense": stats["special-defense"], speed: stats.speed, bst: Object.values(stats).reduce((sum, value) => sum + value, 0), height: data.height, weight: data.weight }]; } } catch {} if (attempt < 2) await new Promise((resolve) => setTimeout(resolve, 300 * (attempt + 1))); } return [key, null]; }
    async function loadStats() { const additions = {}; for (let index = 0; index < targets.length; index += 12) { const rows = await Promise.all(targets.slice(index, index + 12).map(loadOne)); rows.forEach(([key, value]) => { if (value) additions[key] = value; }); if (index + 12 < targets.length) await new Promise((resolve) => setTimeout(resolve, 40)); }
      if (!cancelled && Object.keys(additions).length) setStatLookup((current) => { const next = { ...current, ...additions }; try { const dimensions = Object.fromEntries(Object.entries(next).filter(([, value]) => value?.height != null && value?.weight != null)); localStorage.setItem("draftcenter:pokemon-dimensions:v1", JSON.stringify(dimensions)); } catch {} return next; }); }
  ...
  const results = useMemo(() => [...filteredPokemon].sort((a, b) => {
    ...
    else if (sortBy === "poll_wins" || sortBy === "bracket_championships" || sortBy === "quiz_popular_finishes") {
      const aValue = communityRankingLookup[a.name.toLowerCase().replace(/[^a-z0-9]+/g, "")]?.[sortBy] || 0;
      const bValue = communityRankingLookup[b.name.toLowerCase().replace(/[^a-z0-9]+/g, "")]?.[sortBy] || 0;
  ...
  }).slice(0, resultLimit === "all" ? undefined : resultLimit), [filteredPokemon, statLookup, communityRankingLookup, sortBy, sortDirection, dexNumbers, resultLimit]);
  function toggleSort(column) { if ((column === "height" || column === "weight") && dimensionLoadedCount < filteredPokemon.length) return; if (sortBy === column) setSortDirection((direction) => direction === "asc" ? "desc" : "asc"); else { setSortBy(column); setSortDirection(column === "name" || column === "dex" || column === "generation" || column === "height" || column === "weight" ? "asc" : "desc"); } }
  ...
  function clearFilters() { setQuery(""); setType(""); setGeneration(""); setGame(""); setColor(""); setEggGroup(""); setShape(""); setSortBy("name"); setAbility(""); setAbilityMatches(null); setMessage(""); }
  ...
  async function choose(name, updateLocation = true) {
    ...
    try {
      ...
      const [pokemonResponse, speciesResponse, pinnedPokeApiMoves, supplementalMoveResponse, placementsResponse, dailyThreeResponse, draftProfileResponse, competitiveProfileResponse, tournamentProfileResponse] = await Promise.all([
        ...
        createClient().rpc("get_public_pokemon_competitive_profile", { p_pokemon_key: reference.apiName }),
        createClient().rpc("get_public_pokemon_tournament_profile", { p_pokemon_key: reference.apiName }),
      ]);
