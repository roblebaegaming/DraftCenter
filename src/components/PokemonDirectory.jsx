...
function WidePokemonDirectory(props) {
  ...
  return <main className="pokemon-directory pokemon-directory-wide">
    <header className="pokemon-directory-header"><a href="/?view=dashboard" className="quiet-button"><img src="/draftcenter-logo.png" alt="" style={{ width: 28, height: 28, borderRadius: 7, objectFit: "cover" }} /><span>← DraftCenter home</span></a><div><span className="eyebrow">POKEMON</span><h1>Explore the Pokedex</h1><p className="muted">A stat-first Pokémon browser. Sort any column, select a Pokémon, and study its full game information on the right.</p></div></header>
    <section className="pokemon-directory-layout">
      <aside className="pokemon-search-card pokemon-browser-card">
        <div className="pokemon-browser-controls"><label>Search Pokemon or Dex #<input value={props.query} onChange={(event) => props.setQuery(event.target.value)} placeholder="Pikachu, Garchomp, 681..." autoFocus /></label><label>Type<select value={props.type} onChange={(event) => props.setType(event.target.value)}><option value="">All types</option>{TYPES.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label>Generation<select value={props.generation} onChange={(event) => props.setGeneration(event.target.value)}><option value="">All generations</option>{[1,2,3,4,5,6,7,8,9].map((item) => <option key={item} value={item}>Gen {item}</option>)}</select></label><label>Show<select value={props.resultLimit} onChange={(event) => props.setResultLimit(event.target.value === "all" ? "all" : Number(event.target.value))}><option value={100}>100</option><option value={200}>200</option><option value="all">All</option></select></label><label>Sort by height or weight<select value={["height","weight"].includes(props.sortBy) ? `${props.sortBy}-${props.sortDirection}` : ""} onChange={(event) => { if (!event.target.value) return; const [column, direction] = event.target.value.split("-"); props.setDimensionSort(column, direction); }}><option value="">Choose size sorting</option><option value="height-asc">Height: shortest first</option><option value="height-desc">Height: tallest first</option><option value="weight-asc">Weight: lightest first</option><option value="weight-desc">Weight: heaviest first</option></select></label></div>
      ...
      <section className="pokemon-detail-card pokemon-detail-wide">
        ...
        {props.details && props.species && <>
          ...
          <div className="pokemon-detail-grid"><section><h3>All base stats</h3><div className="pokemon-stats">{props.details.stats.map((stat) => <div key={stat.stat.name}><span>{displayName(stat.stat.name).replace("special", "Sp.")}</span><strong>{stat.base_stat}</strong></div>)}<div className="pokemon-bst-total"><span>BST</span><strong>{props.details.stats.reduce((sum, stat) => sum + stat.base_stat, 0)}</strong></div></div></section><section><h3>Abilities</h3><div className="pokemon-tags">{props.details.abilities.map(({ ability, is_hidden }) => <span key={ability.name}>{displayName(ability.name)}{is_hidden ? " (hidden)" : ""}</span>)}</div><h3>DraftCenter poll podiums</h3><PokemonPollPlacements placements={props.pollPlacements} /></section></div>
...
function PokemonDirectoryContent() {
  ...
  function setDimensionSort(column, direction) { setSortBy(column); setSortDirection(direction); }
  ...
  async function choose(name) {
    setSelected(name); setDetails(null); setSpecies(null); setImportedMoves([]); setPollPlacements(null); setDraftProfile(null); setMessage(""); setMoveSource(""); setMoveCategory("all"); setMoveQuery(""); setSelectedMove(null); setLoading(true);
    try {
      const reference = pokemonReference(name);
      const [pokemonResponse, speciesResponse, importedResponse, placementsResponse, draftProfileResponse] = await Promise.all([
      ...
      setDetails(pokemon); setSpecies(speciesData); setImportedMoves(importedResponse.data || []); setPollPlacements(placementsResponse.data || { first: { count: 0, polls: [] }, second: { count: 0, polls: [] }, third: { count: 0, polls: [] } }); setDraftProfile(draftProfileResponse.data || {});
  ...
  return <WidePokemonDirectory {...{ query, setQuery, type, setType, generation, setGeneration, resultLimit, setResultLimit, ability, setAbility, findAbility, clearFilters, results, selected, choose, dexNumbers, statLookup, sortBy, sortDirection, toggleSort, setDimensionSort, loading, message, details, species, uniqueEntries, regulationStatus, sourceData, activeSource, recommendedSource, moveCategory, setMoveCategory, moveQuery, setMoveQuery, setMoveSource, visibleMoves, groupedMoves, selectedMove, inspectMove, setSelectedMove, loadingMove, loadingMoveCategories, inspectedMove, pollPlacements, draftProfile }} />;
