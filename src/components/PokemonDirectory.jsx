...
function PokemonDirectoryContent() {
  ...
  async function choose(name) {
    setSelected(name); setDetails(null); setSpecies(null); setImportedMoves([]); setPollPlacements(null); setMessage(""); setMoveSource(""); setMoveCategory("all"); setMoveQuery(""); setSelectedMove(null); setLoading(true);
    try {
      const reference = pokemonReference(name);
      const [pokemonResponse, speciesResponse, importedResponse, placementsResponse] = await Promise.all([
      ...
      setDetails(pokemon); setSpecies(speciesData); setImportedMoves(importedResponse.data || []); setPollPlacements(placementsResponse.data || { first: { count: 0, polls: [] }, second: { count: 0, polls: [] }, third: { count: 0, polls: [] } });
