  ...
  return <main className="pokemon-directory pokemon-directory-wide">
    ...
    <section className="pokemon-directory-layout">
      <aside className="pokemon-search-card pokemon-browser-card">
        <div className="pokemon-browser-controls"><label>Search Pokemon<input value={props.query} onChange={(event) => props.setQuery(event.target.value)} placeholder="Pikachu, Garchomp..." autoFocus /></label><label>Type<select value={props.type} onChange={(event) => props.setType(event.target.value)}><option value="">All types</option>{TYPES.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label>Generation<select value={props.generation} onChange={(event) => props.setGeneration(event.target.value)}><option value="">All generations</option>{[1,2,3,4,5,6,7,8,9].map((item) => <option key={item} value={item}>Gen {item}</option>)}</select></label></div>
