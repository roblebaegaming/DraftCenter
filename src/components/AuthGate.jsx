...
function ProfileEditor({ supabase, user, profile, onSaved, onClose }) {
  const [file, setFile] = useState(null); const [message, setMessage] = useState(""); const [busy, setBusy] = useState(false); const [dailyThreeEmail, setDailyThreeEmail] = useState(false); const [deletionRequest,setDeletionRequest]=useState(null); const [deletionEmail,setDeletionEmail]=useState(""); const [deletionPhrase,setDeletionPhrase]=useState(""); const [deletionBlockers,setDeletionBlockers]=useState([]);
...
function LeagueTools({ league, corrections, onClose, onUpdated, onDeleted }) {
  ...
  return <div className="modal-backdrop">
    <section className="tools-modal">
      ...
      {league.role==="commissioner"&&<section className="support-access-panel"><h3>Transfer primary commissioner</h3><p className="muted">The new commissioner must already belong to this league. They receive ownership and the primary commissioner role; you become a co-commissioner. This is required before deleting an account that owns leagues.</p><div className="form-stack"><label>New commissioner<select value={transferUsername} onChange={(e)=>setTransferUsername(e.target.value)}><option value="">Choose a league member</option>{transferCandidates.map(member=><option key={member.membership_id} value={member.username}>{member.display_name||`@${member.username}`}{member.role==='co_commissioner'?" — co-commissioner":""}</option>)}</select></label><label>Type <strong>TRANSFER</strong><input value={transferConfirmation} onChange={(e)=>setTransferConfirmation(e.target.value)} autoComplete="off"/></label><button type="button" className="danger-button" disabled={busy||!transferUsername||transferConfirmation!=="TRANSFER"} onClick={transferCommissioner}>Transfer league ownership</button></div></section>}
      ...
      <div className="form-stack"><label>League member<select value={removeUsername} onChange={(e)=>setRemoveUsername(e.target.value)}><option value="">Choose a league member</option>{assignedRemovableMembers.length>0&&<optgroup label="Managers with teams">{assignedRemovableMembers.map((member)=><option key={member.membership_id} value={member.username}>{member.display_name||`@${member.username}`} — {member.team_name}{member.role==='co_commissioner'?" — co-commissioner":""}</option>)}</optgroup>}{unassignedRemovableMembers.length>0&&<optgroup label="Joined without a team">{unassignedRemovableMembers.map((member)=><option key={member.membership_id} value={member.username}>{member.display_name||`@${member.username}`} — no team claimed{member.role==='co_commissioner'?" — co-commissioner":""}</option>)}</optgroup>}</select></label><button type="button" className="danger-button league-tool-small-action" disabled={busy||!removeUsername} onClick={removeManager}>Remove league member</button></div>
...
export default function AuthGate(){
  ...
  function changeMode(next){setMode(next);setMessage('');setPassword('');setConfirmPassword('');resetCaptcha();}
...
if(session&&mode!=='reset_password'){if(!profile?.username)return <ProfileSetup supabase={supabase} user={session.user} onSaved={setProfile}/>;if(!activeLeague)return <><BadgeAwardPopup supabase={supabase} userId={session.user.id}/><div className="site-account"><a href="/pokemon">Pokémon</a><a href="/explore">Community</a><button onClick={()=>setShowProfile(true)}>Profile</button><button onClick={()=>supabase.auth.signOut()}>Sign out</button><span>@{profile.username}</span></div><LeagueHub user={session.user} profile={profile} onOpenLeague={openLeague}/>{showProfile&&<ProfileEditor supabase={supabase} user={session.user} profile={profile} onSaved={setProfile} onClose={()=>setShowProfile(false)}/>}</>;return <><BadgeAwardPopup supabase={supabase} userId={session.user.id}/><div className="site-account"><button onClick={()=>closeLeague()}>Dashboard</button><a href="/pokemon">Pokémon</a><a href="/explore">Community</a><button onClick={()=>setShowProfile(true)}>Profile</button><button onClick={()=>supabase.auth.signOut()}>Sign out</button><span>@{profile.username}</span></div><LeagueErrorBoundary key={activeLeague.id} onExit={()=>closeLeague()}><PokemonDraftLeague leagueId={activeLeague.id} leagueRole={activeLeague.role} league={activeLeague} profile={profile} onOpenLeagueTools={(corrections)=>{setToolCorrections(corrections);setShowTools(true);}}/></LeagueErrorBoundary>{showTools&&<LeagueTools league={activeLeague} corrections={toolCorrections} onClose={()=>{setShowTools(false);setToolCorrections(null);}} onUpdated={(league)=>openLeague(league,true)} onDeleted={()=>closeLeague(true)}/>} {showProfile&&<ProfileEditor supabase={supabase} user={session.user} profile={profile} onSaved={setProfile} onClose={()=>setShowProfile(false)}/>}</>;}
  ...
  if(mode==='sign_in')return <PublicLanding email={email} password={password} setEmail={setEmail} setPassword={setPassword} busy={busy} message={message} onSubmit={submit} onMode={changeMode} captchaReady={Boolean(captchaToken)} captchaRequired={authCaptchaRequired(TURNSTILE_SITE_KEY,mode,TURNSTILE_ENFORCED)} captchaResetKey={captchaResetKey} onCaptchaToken={setCaptchaToken}/>;
