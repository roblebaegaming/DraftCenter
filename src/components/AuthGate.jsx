...
function VisitorGuide() {
  return <section className="visitor-home-guide"><span className="eyebrow">ONE CONNECTED SEASON</span><h2>From setup to champion in five steps</h2><ol className="visitor-lifecycle"><li><strong>Set the rules</strong><span>Choose the format, teams, pool, prices, and deadlines.</span></li><li><strong>Invite managers</strong><span>Share one link and let each manager claim a team.</span></li><li><strong>Run the draft</strong><span>Snake, budget, or auction—with live boards and recovery.</span></li><li><strong>Play the season</strong><span>Schedules, transactions, results, standings, and playoffs stay together.</span></li><li><strong>Preserve it</strong><span>Export the completed season and carry the league into next year.</span></li></ol><nav className="visitor-commissioner-resources" aria-label="Commissioner resources"><span>Commissioner resources</span><a href="/guides/how-to-run-pokemon-draft-league">Run a league guide</a><a href="/guides/pokemon-draft-manager-vs-spreadsheets">Move from a spreadsheet</a><a href="/guides/pokemon-auction-tournament-swiss-top-cut">Auction, Swiss, and Top Cut</a><a href="/guides/vgc-open-closed-team-sheet-battle-tracker">Track VGC battles</a><a href="/guides/pokemon-showdown-replay-results-draft-league">Showdown replay results</a><a href="/guides/snake-vs-auction-pokemon-draft">Snake or auction?</a></nav></section>;
}
...
function PublicLanding({ email, password, setEmail, setPassword, busy, message, onSubmit, onMode, captchaReady, captchaRequired, captchaResetKey, onCaptchaToken }) {
  ...
  function startCommissionerPath(practice = false) {
    try { localStorage.setItem("draftcenter:commissioner-intent:v1", practice ? "practice" : "league"); } catch {}
    trackActivationEvent(practice ? "practice_path_started" : "commissioner_path_started", { properties: { source: "home", practice: practice ? "yes" : "no" } });
    onMode("sign_up");
  }
  return <main className="visitor-home"><section className="visitor-hero"><div className="visitor-brand"><img src="/draftcenter-logo.png" alt="DraftCenter" /><div><span className="eyebrow">DRAFTCENTER</span><small>at draftcentral.gg</small></div></div><h1>Run your whole Pokémon draft league in one place.</h1><p>Set up, draft, schedule, play, and preserve a complete season without managing it across spreadsheets, Discord-only state, and separate tools.</p><div className="visitor-primary-actions"><button type="button" className="primary-button" onClick={() => startCommissionerPath(false)}>Run a league</button><a className="secondary-button" href="/leagues">Join a league</a><a className="quiet-button" href="/team-lab">Prepare for a match</a></div><button type="button" className="visitor-practice-link" onClick={() => startCommissionerPath(true)}>New commissioner? Start with a private practice league →</button><VisitorGuide /><details className="visitor-explore-more"><summary>Explore DraftCenter’s public Pokémon tools</summary><div className="visitor-free-grid"><a href="/pokemon" className="visitor-feature-card"><span>POKÉDEX</span><strong>Explore Pokémon</strong>{featured?.image && <img src={featured.image} alt={featured.name} />}{featured ? <div className="visitor-feature-copy"><b>Featured: {featured.name}</b>{featured.entry ? <small>“{featured.entry}” <em>{featured.game}</em></small> : <small>Open its full Pokédex entry, stats, and move pools.</small>}</div> : <small>Loading a Pokémon from the Pokédex...</small>}</a><a href="/explore" className="visitor-feature-card"><span>COMMUNITY</span><strong>Explore trends</strong><RotatingPokemonArtwork names={communityPokemon} className="visitor-card-pokemon" /><div className="visitor-poll-preview"><b>Poll of the Day</b><p>{poll?.question || "See today’s Pokémon question and community results."}</p>{poll?.total_votes != null && <small>{poll.total_votes} vote{poll.total_votes === 1 ? "" : "s"} so far</small>}</div></a><a href="/leagues" className="visitor-feature-card"><span>PUBLIC LEAGUES</span><strong>Watch a draft</strong><RotatingPokemonArtwork names={WORLD_CHAMPION_POKEMON} interval={6300} className="visitor-card-pokemon" /><div className="visitor-feature-copy"><b>Find a league</b><small>Join an open team or follow standings, schedules, replays, and predictions.</small></div></a></div></details></section><aside id="member-access" className="visitor-signin"><span className="eyebrow">MEMBERS</span><h2>Welcome back</h2><p className="muted">Sign in to continue your league or accept an invitation.</p><form onSubmit={onSubmit} className="form-stack"><label>Email<input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} style={inputStyle}/></label><label>Password<input type="password" required minLength={6} value={password} onChange={(event) => setPassword(event.target.value)} style={inputStyle}/></label><TurnstileChallenge siteKey={TURNSTILE_SITE_KEY} action="sign_in" resetKey={captchaResetKey} onTokenChange={onCaptchaToken}/>{message && <p className="hub-message">{message}</p>}<button className="primary-button" disabled={busy||Boolean(captchaRequired&&!captchaReady)}>{busy ? "Please wait..." : "Sign in"}</button></form><div className="visitor-account-links"><button className="text-button" onClick={() => onMode("forgot_password")}>Forgot password?</button><button className="text-button" onClick={() => onMode("sign_up")}>New here? Create an account</button></div></aside></main>;
}

function PublicLoadingShell() {
  return <main className="visitor-home"><section className="visitor-hero"><div className="visitor-brand"><img src="/draftcenter-logo.png" alt="DraftCenter" /><div><span className="eyebrow">DRAFTCENTER</span><small>at draftcentral.gg</small></div></div><h1>Run your whole Pokémon draft league in one place.</h1><p>Set up, draft, schedule, play, and preserve a complete season without splitting it across separate tools.</p><div className="visitor-primary-actions"><a className="primary-button" href="#member-access">Run a league</a><a className="secondary-button" href="/leagues">Join a league</a><a className="quiet-button" href="/team-lab">Prepare for a match</a></div><VisitorGuide /></section><aside id="member-access" className="visitor-signin"><span className="eyebrow">MEMBERS</span><h2>Member access</h2><p className="muted">Checking your sign-in status…</p><a className="quiet-button inline-link-button" href="/leagues">Browse public leagues</a></aside></main>;
}
