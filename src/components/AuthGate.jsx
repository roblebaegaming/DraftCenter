...
function ProfileEditor({ supabase, user, profile, onSaved, onClose }) {
  ...
  async function downloadAccountExport() {
    ...
    const requests=[
      ...
      ["league_memberships",supabase.from("league_memberships").select("league_id,role,league:leagues(id,name,slug,season_label,status)").eq("user_id",user.id)],
...
function LeagueTools({ league, corrections, onClose, onUpdated }) {
  const [supabase] = useState(() => createClient());
  ...
  const [invite,setInvite]=useState(""); const [inviteEmail,setInviteEmail]=useState(""); const [coUsername,setCoUsername]=useState(""); const [coEmail,setCoEmail]=useState(""); const [removeUsername,setRemoveUsername]=useState(""); const [members,setMembers]=useState([]); const [message,setMessage]=useState(""); const [busy,setBusy]=useState(false); const [reversedTrades,setReversedTrades]=useState([]); const [reversedMoves,setReversedMoves]=useState([]);
