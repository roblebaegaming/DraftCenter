...
class LeagueErrorBoundary extends Component {
  ...
  componentDidCatch(error) {
    console.error("League screen failed", error);
    if (!this.props.supabase || !this.props.leagueId) return;
    reportOperationalIssue(this.props.supabase, {
      kind: "client_runtime_error",
      message: error?.message || "Unknown league-screen error",
      leagueId: this.props.leagueId,
      context: {
        action: "league_render",
        release: DRAFTCENTER_RELEASE,
        role: this.props.leagueRole || null,
        route: "league",
      },
    });
  }
...
function BadgeAwardPopup({ supabase, userId }) {
  ...
  useEffect(()=>{
    if(!userId)return;
    supabase.rpc("refresh_my_daily_three_badges").then(({data})=>setEvents(data?.events||[]));
...
function ProfileEditor({ supabase, user, profile, onSaved, onClose }) {
  ...
  return <div className="modal-backdrop"><section className="tools-modal profile-tools-modal"><button className="modal-close" onClick={onClose}>x</button><span className="eyebrow">YOUR PROFILE</span><h2>Profile photo</h2>{profile?.avatar_url ? <img className="profile-photo-large" src={profile.avatar_url} alt="Your profile" /> : <div className="profile-photo-placeholder">{(profile?.display_name || profile?.username || "C")[0].toUpperCase()}</div>}<p className="muted">Your photo is visible beside your name in DraftCenter discussions.</p><form className="form-stack" onSubmit={uploadPhoto}><label>Choose photo<input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event)=>setFile(event.target.files?.[0]||null)} /></label><button className="primary-button" disabled={busy}>{busy?"Uploading...":"Save profile photo"}</button></form><CareerMatchRecord supabase={supabase}/><FavoritePokemonEditor supabase={supabase} user={user}/><DiscordProfileConnection supabase={supabase} user={user}/><ProfileBadges supabase={supabase}/><hr/><h3>Daily Three emails</h3><label className="check-row"><input type="checkbox" checked={dailyThreeEmail} disabled={busy} onChange={(event)=>saveDailyThreeEmail(event.target.checked)} /> Email me yesterday’s Poll, Draft Bracket, and Pokémon Quiz results</label><hr/><h3>Your data</h3><p className="muted">Download a private account copy containing your profile, preferences, My Teams workspaces and plans, private league notebooks, memberships, league-team history, Discord connection metadata, and your discussion activity.</p><button type="button" className="secondary-button" disabled={busy} onClick={downloadAccountExport}>{busy?"Preparing export…":"Download private account export"}</button>{message&&<p className="hub-message">{message}</p>}</section></div>;
}

function DiscordSignInButton({ busy, onSignIn }) {
  return <button type="button" className="discord-signin-button" disabled={busy} onClick={onSignIn}>
    <span aria-hidden="true">Discord</span>
    <strong>{busy ? "Opening Discord..." : "Continue with Discord"}</strong>
  </button>;
}

function PublicLanding({ email, password, setEmail, setPassword, busy, message, onSubmit, onMode, onDiscordSignIn }) {
...
function LeagueTools({ league, corrections, onClose, onUpdated, onDeleted }) {
  const [supabase] = useState(() => createClient());
  const [name,setName]=useState(league.name||""); const [season,setSeason]=useState(league.season_label||""); const [description,setDescription]=useState(league.description||""); const [imageUrl,setImageUrl]=useState(league.image_url||""); const [startsAt,setStartsAt]=useState(league.draft_starts_at ? new Date(league.draft_starts_at).toISOString().slice(0,16) : ""); const [visibility,setVisibility]=useState(league.league_visibility||"private");
  const [invite,setInvite]=useState(""); const [inviteEmail,setInviteEmail]=useState(""); const [coUsername,setCoUsername]=useState(""); const [coEmail,setCoEmail]=useState(""); const [removeUsername,setRemoveUsername]=useState(""); const [members,setMembers]=useState([]); const [message,setMessage]=useState(""); const [busy,setBusy]=useState(false); const [reversedTrades,setReversedTrades]=useState([]); const [reversedMoves,setReversedMoves]=useState([]); const [deleteConfirmation,setDeleteConfirmation]=useState("");
  function reportCommissionerFailure(action, error) {
    reportOperationalIssue(supabase, {
      kind: "commissioner_action_failed",
      message: error?.message || String(error || "Commissioner action failed"),
      leagueId: league.id,
      context: { action, role: league.role, route: "league_tools" },
    });
  }
  ...
  async function saveDetails(event) { event.preventDefault(); setBusy(true); setMessage(""); const imageResult=await supabase.rpc('update_league_image',{p_league_id:league.id,p_image_url:imageUrl.trim()||null}); if(imageResult.error){reportCommissionerFailure("update_league_image",imageResult.error);setBusy(false);return setMessage(imageResult.error.message);} const {data,error}=await supabase.rpc('update_league_details',{p_league_id:league.id,p_name:name,p_description:description,p_season_label:season,p_draft_starts_at:startsAt ? new Date(startsAt).toISOString():null,p_is_public:visibility!=="private"}); if(error){reportCommissionerFailure("update_league_details",error);setBusy(false);return setMessage(error.message);} const accessResult=await supabase.rpc('update_league_access',{p_league_id:league.id,p_visibility:visibility,p_is_practice:Boolean(league.is_practice),p_practice_expires_at:league.practice_expires_at||null}); if(accessResult.error){reportCommissionerFailure("update_league_access",accessResult.error);setBusy(false);return setMessage(`League details saved, but public access could not be updated: ${accessResult.error.message}`);} let note=""; if(startsAt){const {data:count,error:reminderError}=await supabase.rpc('schedule_draft_reminders',{p_league_id:league.id});if(reminderError)reportCommissionerFailure("schedule_draft_reminders",reminderError);note=reminderError ? ' Draft reminders will need configuration first.' : ` ${count||0} reminder jobs scheduled.`;note += " Open Draft Setup and wait for AUTOMATIC START READY before leaving.";} setBusy(false);onUpdated({...league,...data,...accessResult.data,image_url:imageResult.data?.image_url||null,league_visibility:visibility});setMessage(`${visibility==="watch"?"League is now listed under Open to Watch.":visibility==="open"?"League is now listed under Open to Join.":"League is now private."}${note}`); }
  async function createLink(kind, openEmail=false) { const email=inviteEmail.trim().toLowerCase(); if(openEmail&&!email)return setMessage('Enter an email address first.'); setBusy(true); const {data,error}=await supabase.rpc(kind==='spectator'?'create_spectator_invite':'create_league_invite',{p_league_id:league.id,p_email:openEmail ? email : null}); setBusy(false); if(error){reportCommissionerFailure(kind==="spectator"?"create_spectator_invite":"create_league_invite",error);return setMessage(error.message);} const link=`${window.location.origin}?${kind==='spectator'?'spectate':'invite'}=${data.token}`; setInvite(link); try{await navigator.clipboard.writeText(link);}catch{} if(openEmail){const subject=encodeURIComponent(`${kind==='spectator'?'Watch':'Join'} ${league.name} on DraftCenter`);const body=encodeURIComponent(`${kind==='spectator'?'You have been invited to watch':'You have been invited to join'} ${league.name} on DraftCenter.\n\n${link}`);window.location.href=`mailto:${encodeURIComponent(email)}?subject=${subject}&body=${body}`;setMessage('Your email app is opening with the link. If nothing opens, copy the link below and send it from your usual email service.');}else setMessage(kind==='spectator'?'Spectator link copied. It grants view-only access for 90 days.':'Manager invite copied. It grants team/league access for 14 days.'); }
  async function promoteCo(){const username=coUsername.trim().toLowerCase().replace(/^@/,"");if(!username)return setMessage('Enter a username first.');setBusy(true);const {error}=await supabase.rpc('set_co_commissioner',{p_league_id:league.id,p_username:username,p_enabled:true});setBusy(false);if(error){reportCommissionerFailure("set_co_commissioner",error);return setMessage(error.message);}setCoUsername('');setMessage(`@${username} is now a co-commissioner.`);}
  async function inviteCoCommissioner(){const email=coEmail.trim().toLowerCase();if(!email)return setMessage('Enter the co-commissioner’s email address first.');setBusy(true);const {data,error}=await supabase.rpc('create_co_commissioner_invite',{p_league_id:league.id,p_email:email});setBusy(false);if(error){reportCommissionerFailure("create_co_commissioner_invite",error);return setMessage(error.message);}const link=`${window.location.origin}?invite=${data.token}`;setInvite(link);try{await navigator.clipboard.writeText(link);}catch{}const subject=encodeURIComponent(`Co-commissioner invitation for ${league.name}`);const body=encodeURIComponent(`You have been invited to help run ${league.name} as a co-commissioner on DraftCenter.\n\nSign in with ${email} and accept here:\n${link}`);window.location.href=`mailto:${encodeURIComponent(email)}?subject=${subject}&body=${body}`;setMessage('Your email app is opening with the secure acceptance link. The link was also copied and is shown below.');}
  async function removeManager(){const username=removeUsername.trim().toLowerCase().replace(/^@/,"");if(!username)return setMessage('Enter the manager\'s username first.');if(!window.confirm(`Remove @${username} from this league? Their team will become available for a replacement.`))return;setBusy(true);const {error}=await supabase.rpc('remove_league_manager',{p_league_id:league.id,p_username:username});setBusy(false);if(error){reportCommissionerFailure("remove_league_manager",error);return setMessage(error.message);}setRemoveUsername('');setMessage(`Removed @${username}. Their team is available for a replacement.`);}
  async function deleteLeaguePermanently(){
    ...
    setBusy(false);
    if(error){reportCommissionerFailure("delete_league",error);return setMessage(error.message);}
    onDeleted?.();
  }
  ...
  return <div className="modal-backdrop">
    <section className="tools-modal">
      ...
      <div className="form-stack"><label>League member<select value={removeUsername} onChange={(e)=>setRemoveUsername(e.target.value)}><option value="">Choose a manager</option>{removableMembers.map((member)=><option key={member.membership_id} value={member.username}>{member.display_name||`@${member.username}`}{member.team_name?` — ${member.team_name}`:""}{member.role==='co_commissioner'?" — co-commissioner":""}</option>)}</select></label><button type="button" className="danger-button league-tool-small-action" disabled={busy||!removeUsername} onClick={removeManager}>Remove manager</button></div>
...
export default function AuthGate(){
  ...
  useEffect(()=>{const params=new URLSearchParams(window.location.search);const authError=params.get("auth_error");if(authError)setMessage(authError);},[]);
  async function signInWithDiscord(){setBusy(true);setMessage('');const {data,error}=await supabase.auth.signInWithOAuth({provider:'discord',options:{redirectTo:authCallbackUrl(window.location.origin),skipBrowserRedirect:true}});if(error||!data?.url){setBusy(false);return setMessage(errorText(error,'Discord sign-in could not start. Please try again.'));}window.location.assign(data.url);}
...
if(session&&mode!=='reset_password'){if(!profile?.username)return <ProfileSetup supabase={supabase} user={session.user} onSaved={setProfile}/>;if(!activeLeague)return <><BadgeAwardPopup supabase={supabase} userId={session.user.id}/><div className="site-account"><a href="/pokemon">Pokémon</a><a href="/explore">Community</a><a href="/calendar">Calendar</a><button onClick={()=>setShowFeedback(true)}>Report issue</button><button onClick={()=>setShowProfile(true)}>Profile</button><button onClick={()=>supabase.auth.signOut()}>Sign out</button><span>@{profile.username}</span></div><LeagueHub user={session.user} profile={profile} onOpenLeague={openLeague}/>{showProfile&&<ProfileEditor supabase={supabase} user={session.user} profile={profile} onSaved={setProfile} onClose={()=>setShowProfile(false)}/>} {showFeedback&&<TesterFeedback supabase={supabase} profile={profile} league={null} onClose={()=>setShowFeedback(false)}/>}</>;return <><BadgeAwardPopup supabase={supabase} userId={session.user.id}/><div className="site-account site-account-league"><button onClick={()=>closeLeague()}>Dashboard</button><a className="site-account-secondary" href="/pokemon">Pokémon</a><a className="site-account-secondary" href="/explore">Community</a><a href="/calendar">Calendar</a><button onClick={()=>setShowFeedback(true)}><span className="account-label-full">Report issue</span><span className="account-label-short">Feedback</span></button><button onClick={()=>setShowProfile(true)}>Profile</button><button onClick={()=>supabase.auth.signOut()}>Sign out</button><span>@{profile.username}</span></div><LeagueErrorBoundary key={activeLeague.id} onExit={()=>closeLeague()} supabase={supabase} leagueId={activeLeague.id} leagueRole={activeLeague.role}><PokemonDraftLeague leagueId={activeLeague.id} leagueRole={activeLeague.role} league={activeLeague} profile={profile} onOpenLeagueTools={(corrections)=>{setToolCorrections(corrections);setShowTools(true);}}/></LeagueErrorBoundary>{showTools&&<LeagueTools league={activeLeague} corrections={toolCorrections} onClose={()=>{setShowTools(false);setToolCorrections(null);}} onUpdated={(league)=>openLeague(league,true)} onDeleted={()=>closeLeague(true)}/>} {showProfile&&<ProfileEditor supabase={supabase} user={session.user} profile={profile} onSaved={setProfile} onClose={()=>setShowProfile(false)}/>} {showFeedback&&<TesterFeedback supabase={supabase} profile={profile} league={activeLeague} onClose={()=>setShowFeedback(false)}/>}</>;}
  ...
  if(mode==='sign_in')return <PublicLanding email={email} password={password} setEmail={setEmail} setPassword={setPassword} busy={busy} message={message} onSubmit={submit} onMode={changeMode} onDiscordSignIn={signInWithDiscord}/>;
