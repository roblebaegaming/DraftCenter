...
function LeagueTools({ league, onClose, onUpdated }) {
  ...
  const [invite,setInvite]=useState(""); const [inviteEmail,setInviteEmail]=useState(""); const [coUsername,setCoUsername]=useState(""); const [coEmail,setCoEmail]=useState(""); const [removeUsername,setRemoveUsername]=useState(""); const [members,setMembers]=useState([]); const [message,setMessage]=useState(""); const [busy,setBusy]=useState(false);
  ...
  return <div className="modal-backdrop">
    <section className="tools-modal">
      ...
      <hr/><section className="league-tool-section"><h3>Transaction corrections</h3><p className="muted">Managers can withdraw their pending claims and cancel their own pending trades from Transactions. Commissioners can also reverse eligible completed trades and free-agent moves from the history there. Safeguards prevent reversal after the involved Pokémon has moved again.</p></section>
