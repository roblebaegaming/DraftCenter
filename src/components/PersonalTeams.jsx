...
const EMPTY = { team_name:"", league_name:"", format_name:"", workspace_type:"weekly", planning_entries:[], notes:"", weekly_notes:"", pokepaste_url:"", replica_code:"", spreadsheet_url:"", pokemon:[], archived:false, is_public:false, regulation_id:"", public_summary:"" };
...
export default function PersonalTeams() {
  ...
  async function save(event) {
    ...
    const payload={owner_id:user.id,team_name:form.team_name.trim(),league_name:nullable(form.league_name),format_name:nullable(form.format_name),workspace_type:form.workspace_type,planning_entries:planningEntries,notes:form.notes.trim(),weekly_notes:"",pokepaste_url:nullable(form.pokepaste_url),replica_code:form.replica_code.trim(),spreadsheet_url:nullable(form.spreadsheet_url),pokemon:form.pokemon,archived:Boolean(form.archived),is_public:Boolean(form.is_public),regulation_id:nullable(form.regulation_id),public_summary:form.public_summary?.trim()||""};
  ...
  async function restorePrivateBackup(event) {
    ...
    try {
      ...
      const rows=parsed.personal_teams.map((team)=>({
        ...
        spreadsheet_url:nullable(team.spreadsheet_url),pokemon:Array.isArray(team.pokemon)?team.pokemon:[],archived:Boolean(team.archived),is_public:Boolean(team.is_public),regulation_id:nullable(team.regulation_id),public_summary:String(team.public_summary||""),
      }));
