...
export default function PersonalTeams() {
  ...
  return <main className="personal-teams-shell">
    ...
    <div className="personal-team-grid">{visible.map((team)=>{const pokemon=cardPokemon(team);return <article className="personal-team-card" key={team.id} onClick={()=>setViewing(team)}><span className="eyebrow">{team.league_name||"PERSONAL TEAM"}</span><h2>{team.team_name}</h2>{team.format_name&&<p className="personal-team-format">{team.format_name}</p>}<span className="personal-team-use-badge">{workspaceLabel(team)}</span><div className="personal-team-pokemon">{pokemon.slice(0,12).map((name,index)=><span key={`${name}-${index}`}>{name}</span>)}{pokemon.length>12&&<span>+{pokemon.length-12} more</span>}{!pokemon.length&&<span className="muted">No Pokémon added</span>}</div><div className="personal-team-links">{team.pokepaste_url&&<a href={team.pokepaste_url} target="_blank" rel="noreferrer" onClick={(event)=>event.stopPropagation()}>PokéPaste ↗</a>}{team.spreadsheet_url&&<a href={team.spreadsheet_url} target="_blank" rel="noreferrer" onClick={(event)=>event.stopPropagation()}>Spreadsheet ↗</a>}{team.team_report_url&&<a href={team.team_report_url} target="_blank" rel="noreferrer" onClick={(event)=>event.stopPropagation()}>{isNuzlockeTeam(team)?"Open run":"Team report"} ↗</a>}</div><div className="personal-team-actions"><button className="secondary-button" onClick={(event)=>{event.stopPropagation();setViewing(team);}}>{isNuzlockeTeam(team)?"View Run Card":"View roster"}</button><button className="text-button danger-text" disabled={busy} onClick={(event)=>{event.stopPropagation();remove(team);}}>Delete</button></div></article>})}</div>
    </section>
    {viewing&&<div className="modal-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)setViewing(null);}}><section className="tools-modal personal-team-viewer"><button className="modal-close" onClick={()=>setViewing(null)}>x</button><span className="eyebrow">{viewing.league_name||"PRIVATE TEAM WORKSPACE"}</span><div className="personal-team-viewer-heading"><div><h2>{viewing.team_name}</h2>{viewing.format_name&&<p className="personal-team-format">{viewing.format_name}</p>}</div>{viewing.league_source?<a className="secondary-button inline-link-button" href={`/?league=${encodeURIComponent(viewing.slug||viewing.league_id)}`}>{viewing.archived?"Open league history":"Open league"}</a>:<div className="personal-team-actions">{isNuzlockeTeam(viewing)&&viewing.team_report_url&&<a className="primary-button inline-link-button" href={viewing.team_report_url}>Open run</a>}<button className="secondary-button" onClick={()=>start(viewing)}>Edit workspace</button></div>}</div>
      {isNuzlockeTeam(viewing)?<><section className="personal-nuzlocke-summary"><strong>{viewing.nuzlocke_run.team.length} encounter{viewing.nuzlocke_run.team.length===1?"":"s"}</strong><span>Team code: {viewing.nuzlocke_run.seed}</span></section>{Array.isArray(viewing.nuzlocke_run.rules)&&viewing.nuzlocke_run.rules.length>0&&<div className="personal-nuzlocke-rules">{viewing.nuzlocke_run.rules.map((rule,index)=><span key={`${rule}-${index}`}>{rule}</span>)}</div>}<div className="personal-nuzlocke-grid">{viewing.nuzlocke_run.team.map((entry,index)=>{const artwork=safeNuzlockeArtworkUrl(entry.artwork_url);return <article key={`${entry.area_key}-${entry.pokemon_id}-${index}`}>{artwork&&<img src={artwork} alt={`${nuzlockeDisplayName(entry)} artwork`}/>}<span>{index+1}</span><div><h3>{nuzlockeDisplayName(entry)}</h3><strong>{entry.area_name}</strong><p>{entry.method==="starter"?"Starter Pokémon":<>{titleCase(entry.method)} · Lv. {entry.min_level??"?"}{entry.max_level!=null&&entry.max_level!==entry.min_level?`–${entry.max_level}`:""}</>}</p>{entry.conditions?.length>0&&<small>{entry.conditions.map(titleCase).join(", ")}</small>}</div></article>})}</div></>:<><div className="personal-roster-grid">{rosterFor(viewing).map((mon,index)=><article key={`${mon.name}-${index}`} className="personal-roster-mon"><MonSprite mon={mon} size={78}/><div><h3>{mon.name}</h3><div className="personal-roster-types"><span className={`type-${mon.t1}`}>{mon.t1}</span>{mon.t2&&<span className={`type-${mon.t2}`}>{mon.t2}</span>}</div><MonStats mon={mon}/><MonAbilities mon={mon} className="personal-roster-abilities"/><div className="personal-roster-defense"><strong>Defensive matchups</strong><MonDefenseChart mon={mon}/></div></div></article>)}</div>{!rosterFor(viewing).length&&<p className="muted">No Pokémon are on this roster yet. Edit the workspace to add them.</p>}{rosterFor(viewing).length>0&&<section className="personal-team-defense-summary"><h3>Team defensive coverage</h3><TeamDefenseSummary roster={rosterFor(viewing)}/></section>}</>}
    ...
    {editing&&<div className="modal-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)cancel();}}><section className="tools-modal personal-team-editor"><button className="modal-close" onClick={cancel}>x</button><span className="eyebrow">{editing==="new"?"NEW PERSONAL TEAM":"PRIVATE TEAM WORKSPACE"}</span><h2>{editing==="new"?"Add a team":form.team_name}</h2><form className="form-stack" onSubmit={save}>
      ...
      {form.workspace_type==="nuzlocke"?<section className="personal-nuzlocke-editor-note"><strong>Generated encounter roster</strong><p className="muted">The routes, encounter methods, levels, and Team code stay attached to this private Run Card. Use Open run to build a fresh result from its saved rules.</p></section>:<div><strong>Team roster</strong><p className="muted">Add every Pokémon on this team, in roster order. Draft teams are not limited to six.</p><div className="personal-roster-builder"><input list="personal-team-pokemon-options" value={pokemonChoice} onChange={(e)=>setPokemonChoice(e.target.value)} placeholder="Search for a Pokémon" autoComplete="off"/><datalist id="personal-team-pokemon-options">{POLL_POKEMON_NAMES.map((name)=><option key={name} value={name}/>)}</datalist><button type="button" className="secondary-button" onClick={addPokemon}>Add to roster</button></div><div className="personal-roster-selections">{form.pokemon.map((name,index)=><span key={`${name}-${index}`}><b>{index+1}</b>{name}<button type="button" aria-label={`Remove ${name}`} onClick={()=>setForm({...form,pokemon:form.pokemon.filter((_,itemIndex)=>itemIndex!==index)})}>x</button></span>)}</div></div>}
      {form.workspace_type!=="nuzlocke"&&<fieldset className="personal-team-sharing"><legend>Community team repository</legend><label className="check-row"><input type="checkbox" checked={form.is_public} onChange={(e)=>setForm((current)=>({...current,is_public:e.target.checked}))}/> Share this roster publicly</label><p className="muted">Sharing is off by default. Your team name, regulation, roster, public summary, and public coach identity are included. Optional attachments require separate permission below.</p>{form.is_public&&<><label>Regulation<select required value={form.regulation_id||""} onChange={(e)=>setForm((current)=>({...current,regulation_id:e.target.value}))}><option value="">Choose a regulation</option>{REGULATION_OPTIONS.map((option)=><option key={option.id} value={option.id}>{option.name}</option>)}</select></label><label>Public summary<textarea maxLength={500} rows={3} placeholder="What is this team designed to do?" value={form.public_summary||""} onChange={(e)=>setForm((current)=>({...current,public_summary:e.target.value}))}/></label></>}</fieldset>}
