...
const EMPTY = { team_name:"", league_name:"", format_name:"", workspace_type:"weekly", planning_entries:[], notes:"", weekly_notes:"", pokepaste_url:"", replica_code:"", spreadsheet_url:"", team_report_url:"", pokemon:[], archived:false, is_public:false, regulation_id:"", public_summary:"", share_pokepaste:false, share_replica_code:false, share_team_report:false };
...
export default function PersonalTeams() {
  ...
  const pokemonMatches = pokemonChoice.trim()
    ? POKEMON_DIRECTORY
        .filter((pokemon)=>pokemon.name.toLowerCase().includes(pokemonChoice.trim().toLowerCase())&&!form.pokemon.includes(pokemon.name))
        .sort((a,b)=>a.name.localeCompare(b.name))
        .slice(0,18)
    : [];
  function cancel() { setEditing(null); setForm(EMPTY); setMessage(""); }
  function addPokemonName(name) {
    const picked=POLL_POKEMON_NAMES.find((pokemonName)=>pokemonName.toLowerCase()===String(name||"").trim().toLowerCase());
  ...
  function addPokemon() { addPokemonName(pokemonChoice); }
  ...
  async function save(event) {
    ...
    const payload={owner_id:user.id,team_name:form.team_name.trim(),league_name:nullable(form.league_name),format_name:nullable(form.format_name),workspace_type:form.workspace_type,planning_entries:planningEntries,notes:form.notes.trim(),weekly_notes:"",pokepaste_url:nullable(form.pokepaste_url),replica_code:form.replica_code.trim(),spreadsheet_url:nullable(form.spreadsheet_url),team_report_url:nullable(form.team_report_url),pokemon:form.pokemon,archived:Boolean(form.archived),is_public:Boolean(form.is_public),regulation_id:nullable(form.regulation_id),public_summary:form.public_summary?.trim()||"",share_pokepaste:Boolean(form.share_pokepaste&&form.pokepaste_url),share_replica_code:Boolean(form.share_replica_code&&form.replica_code.trim()),share_team_report:Boolean(form.share_team_report&&form.team_report_url)};
  ...
  async function restorePrivateBackup(event) {
    ...
    try {
      ...
      const rows=parsed.personal_teams.map((team)=>({
        ...
        share_pokepaste:Boolean(team.share_pokepaste),share_replica_code:Boolean(team.share_replica_code),share_team_report:Boolean(team.share_team_report),
      }));
  ...
  return <main className="personal-teams-shell">
    ...
    {viewing&&<div className="modal-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)setViewing(null);}}><section className="tools-modal personal-team-viewer"><button className="modal-close" onClick={()=>setViewing(null)}>x</button><span className="eyebrow">{viewing.league_name||"PRIVATE TEAM WORKSPACE"}</span><div className="personal-team-viewer-heading"><div><h2>{viewing.team_name}</h2>{viewing.format_name&&<p className="personal-team-format">{viewing.format_name}</p>}</div>{viewing.league_source?<a className="secondary-button inline-link-button" href={`/?league=${encodeURIComponent(viewing.slug||viewing.league_id)}`}>{viewing.archived?"Open league history":"Open league"}</a>:<button className="secondary-button" onClick={()=>start(viewing)}>Edit workspace</button>}</div>
      ...
      {rosterFor(viewing).length>0&&<section className="personal-team-defense-summary"><h3>Team defensive coverage</h3><TeamDefenseSummary roster={rosterFor(viewing)}/></section>}
      ...
      <div className="personal-team-links">{viewing.pokepaste_url&&<a href={viewing.pokepaste_url} target="_blank" rel="noreferrer">PokéPaste ↗</a>}{viewing.spreadsheet_url&&<a href={viewing.spreadsheet_url} target="_blank" rel="noreferrer">Spreadsheet ↗</a>}{viewing.team_report_url&&<a href={viewing.team_report_url} target="_blank" rel="noreferrer">Team report ↗</a>}</div>
    ...
    {editing&&<div className="modal-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)cancel();}}><section className="tools-modal personal-team-editor"><button className="modal-close" onClick={cancel}>x</button><span className="eyebrow">{editing==="new"?"NEW PERSONAL TEAM":"PRIVATE TEAM WORKSPACE"}</span><h2>{editing==="new"?"Add a team":form.team_name}</h2><form className="form-stack" onSubmit={save}>
      ...
      <div><strong>Team roster</strong><p className="muted">Search by name, then select a Pokémon card to add it. Draft teams are not limited to six.</p><div className="personal-roster-builder"><input value={pokemonChoice} onChange={(e)=>setPokemonChoice(e.target.value)} onKeyDown={(event)=>{if(event.key==="Enter"){event.preventDefault();addPokemon();}}} placeholder="Search for a Pokémon" autoComplete="off"/><button type="button" className="secondary-button" onClick={addPokemon}>Add exact match</button></div>{pokemonMatches.length>0&&<div className="personal-pokemon-picker" role="listbox" aria-label="Matching Pokémon">{pokemonMatches.map((pokemon)=><button type="button" role="option" aria-selected="false" key={pokemon.name} onClick={()=>addPokemonName(pokemon.name)}><MonSprite mon={pokemon} size={46}/><span><strong>{pokemon.name}</strong><small>{pokemon.t1}{pokemon.t2?` / ${pokemon.t2}`:""}</small></span><b>+</b></button>)}</div>}<div className="personal-roster-selections">{form.pokemon.map((name,index)=><span key={name}><b>{index+1}</b>{name}<button type="button" aria-label={`Remove ${name}`} onClick={()=>setForm({...form,pokemon:form.pokemon.filter((item)=>item!==name)})}>x</button></span>)}</div></div>
      <fieldset className="personal-team-sharing"><legend>Community team repository</legend><label className="check-row"><input type="checkbox" checked={form.is_public} onChange={(e)=>setForm((current)=>({...current,is_public:e.target.checked}))}/> Share this roster publicly</label><p className="muted">Sharing is off by default. Your team name, regulation, roster, public summary, and public coach identity are included. Optional attachments require separate permission below.</p>{form.is_public&&<><label>Regulation<select required value={form.regulation_id||""} onChange={(e)=>setForm((current)=>({...current,regulation_id:e.target.value}))}><option value="">Choose a regulation</option>{REGULATION_OPTIONS.map((option)=><option key={option.id} value={option.id}>{option.name}</option>)}</select></label><label>Public summary<textarea maxLength={500} rows={3} placeholder="What is this team designed to do?" value={form.public_summary||""} onChange={(e)=>setForm((current)=>({...current,public_summary:e.target.value}))}/></label></>}</fieldset>
