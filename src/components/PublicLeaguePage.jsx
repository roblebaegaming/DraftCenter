...
export default function PublicLeaguePage() {
  ...
  return <main className="explore-shell">
    ...
    {data?.league && <>
      ...
      <section className="explore-card"><h2>League clock</h2>{settings.calendarMode === "weekly" ? <div className="public-pick-list"><div><strong>Season start</strong><span>{settings.seasonStartsAt ? new Date(settings.seasonStartsAt).toLocaleString() : "Not set"}</span></div><div><strong>Matchup week closes</strong><span>{DAYS[settings.matchDayOfWeek]} at {settings.matchTime} · {settings.leagueTimeZone}</span></div><div><strong>Claims process</strong><span>{DAYS[settings.claimDayOfWeek]} at {settings.claimTime} · {settings.leagueTimeZone}</span></div></div> : <p className="muted">This league uses an untimed commissioner-led schedule.</p>}</section>
