...
function initialSetup(fieldSize = 16) {
  ...
  return {
    ...
    locks_at: localDateTime(new Date(now.getTime() + 30 * 60_000)),
...
function emptyEvent() {
  return { display_name: "", event_id: "", description: "", official_info_url: "", confirmation_text: "" };
}

function effectiveState(bracket) {
  if (!bracket?.revision) return "private setup";
...
export default function BracketChallengeOperations() {
  ...
  useEffect(() => {
    ...
    try { window.localStorage.setItem(`${SETUP_DRAFT_PREFIX}${selectedEventId}`, JSON.stringify(setup)); } catch { /* Optional local backup. */ }
  ...
  async function createEvent(event) {
    ...
    try {
      ...
      setMessage(`Event created privately. Its public link will be ${result.public_path} after you publish the reviewed bracket.`);
    } catch (creationError) {
  ...
  function applyParticipantPaste(text = participantPaste) {
    try {
      ...
      setError("");
      setMessage(`${parsed.fieldSize} players loaded into a ${parsed.capacity}-slot bracket. Review every matchup before publishing.`);
    } catch (pasteError) { setError(pasteError.message); }
  }
  ...
  function publish(event, supersede = false) {
    ...
    }, supersede ? "The sole owner entry was archived and the reviewed replacement bracket is live." : "The reviewed bracket is published and its prediction page is live.");
  }
  ...
  let publicPath = "";
  try { if (selectedEventId) publicPath = predictionBracketEventPath(selectedEventId); } catch { publicPath = ""; }

  return <section className="worlds-results-operations worlds-bracket-operations" id="prediction-event-publisher">
    <header><div><span className="eyebrow">OWNER ONLY · TOURNAMENT PREDICTIONS</span><h2>Prediction bracket publisher</h2><p>Create a private setup, load the reviewed official field, publish one permanent link, and score it from confirmed results.</p></div><button className="quiet-button" disabled={busy || loading} onClick={() => loadList(selectedEventId).catch((loadError) => setError(loadError.message))}>Refresh</button></header>
    ...
    <details className="worlds-ops-panel prediction-event-create" open={!events.length}>
      <summary>Create a custom prediction event</summary>
      <form className="worlds-ops-form" onSubmit={createEvent}>
        <label>Event name<input required minLength="3" maxLength="120" value={newEvent.display_name} onChange={(event) => { const display_name = event.target.value; setNewEvent({ ...newEvent, display_name, event_id: slugEdited ? newEvent.event_id : predictionBracketEventSlug(display_name) }); }} placeholder="Regional Championship Top Cut" /></label>
        <label>Public URL name<input required minLength="3" maxLength="80" pattern="[a-z0-9-]+" value={newEvent.event_id} onChange={(event) => { setSlugEdited(true); setNewEvent({ ...newEvent, event_id: predictionBracketEventSlug(event.target.value) }); }} /><small>draftcentral.gg/tournaments/predictions/{newEvent.event_id || "your-event"}</small></label>
        ...
        <div className="wide worlds-ops-actions"><button className="primary-button" disabled={busy || newEvent.confirmation_text !== "CREATE PREDICTION EVENT"}>Create private setup</button><small>Nothing appears publicly until the reviewed bracket is published.</small></div>
      </form>
    </details>

    {events.length > 0 && <div className="prediction-event-switcher">
      <label>Event<select value={selectedEventId} onChange={(event) => chooseEvent(event.target.value)}>{events.map((item) => <option key={item.event_id} value={item.event_id}>{item.display_name} · {item.revision ? effectiveState(item) : "private setup"}</option>)}</select></label>
      <div>{published && publicPath && <a className="quiet-button" href={publicPath} target="_blank" rel="noreferrer">Open public page ↗</a>}{publicPath && <button className="quiet-button" type="button" onClick={() => navigator.clipboard?.writeText(`${window.location.origin}${publicPath}`).then(() => setMessage("Prediction link copied."))}>Copy link</button>}</div>
    </div>}
    ...
    {loading ? <p>Loading prediction events…</p> : data && <>
      ...
      <details className="worlds-ops-panel" open={!published}>
        ...
        {!canReplace ? <p>The field is immutable because multiple entries, an official result, or finalization already exists.</p> : <form className="worlds-ops-form worlds-bracket-setup" onSubmit={(event) => publish(event, canSupersede)}>
          {canSupersede && <p className="wide worlds-ops-warning">Exactly one entry exists and it must belong to the approving owner. Superseding archives that entry and opens a fresh revision.</p>}
          <div className="wide prediction-bulk-import">
            <header><div><strong>Fast field import</strong><p>Paste rows in official slot order: slot, player, country, seed. Tabs, commas, and pipes are supported; use BYE for an official empty slot.</p></div><div><label className="quiet-button file-button">Choose file<input type="file" accept=".tsv,.csv,.txt,text/tab-separated-values,text/csv,text/plain" onChange={(event) => importParticipantFile(event.target.files?.[0]).catch((fileError) => setError(fileError.message))} /></label><button className="quiet-button" type="button" onClick={downloadTemplate}>Download template</button></div></header>
          ...
          <div className="wide worlds-official-pairings">
            <header><div><strong>Final matchup review</strong><p>Compare every slot to the official bracket. Leave only official bye slots empty.</p></div></header>
            {Array.from({ length: setup.bracket_capacity / 2 }, (_, matchIndex) => <article key={matchIndex}>
              ...
              {[matchIndex * 2, matchIndex * 2 + 1].map((participantIndex) => <div key={participantIndex}>
                <label>Slot {participantIndex + 1}<input value={setup.participants[participantIndex]?.display_name || ""} onChange={(event) => updateParticipant(participantIndex, { display_name: event.target.value })} placeholder="Player or leave blank for a bye" /></label>
      ...
      {canCarryForward && <details className="worlds-ops-panel" open><summary>Carry forward the archived owner bracket</summary><div className="worlds-finalize-form"><p>This preserves the archived bracket-side choices in the replacement field without changing any official result.</p><label>Type <strong>CARRY FORWARD ARCHIVED OWNER ENTRY</strong><input value={carryConfirmation} onChange={(event) => setCarryConfirmation(event.target.value)} /></label><button className="primary-button" disabled={busy || carryConfirmation !== "CARRY FORWARD ARCHIVED OWNER ENTRY"} onClick={() => mutate({ action: "carry_forward", source_revision: archivedRevision, confirmation_text: carryConfirmation }, "The archived owner bracket is now a labeled carryover entry.")}>Create historical carryover</button></div></details>}
      ...
      {published && <details className="worlds-ops-panel" open={locked && bracket.status !== "final"}>
        ...
        {!locked ? <p>Winner controls stay closed until entries lock at <strong>{when(bracket.locks_at)}</strong>.</p> : <><label className="worlds-result-source">Official live bracket URL<input required type="url" value={resultSourceUrl} onChange={(event) => setResultSourceUrl(event.target.value)} /></label><div className="worlds-owner-result-rounds">{resultRounds.map((round, roundIndex) => <section key={roundIndex}><h3>Round {roundIndex + 1} · {bracket.round_points[String(roundIndex + 1)]} pts</h3>{round.map((match) => <article key={match.key}><span>Match {match.match}</span>{match.isBye ? <small>{match.automaticWinner.displayName} advances with a bye</small> : match.a && match.b ? <div>{[match.a, match.b].map((competitor) => <button type="button" className={match.result?.winner_id === competitor.id ? "is-winner" : ""} disabled={busy || !resultSourceUrl || bracket.status === "final"} key={competitor.id} onClick={() => mutate({ action: "record_result", round_number: match.round, match_number: match.match, winner_id: competitor.id, source_url: resultSourceUrl }, `${competitor.displayName} recorded as the reviewed winner.`)}>{competitor.displayName}</button>)}</div> : <small>Record feeder winners first</small>}</article>)}</section>)}</div></>}
      </details>}

      {published && <details className="worlds-ops-panel finalization" open={allResults && bracket.status !== "final"}><summary>Finalize the challenge</summary>{bracket.status === "final" ? <p><strong>Finalized {when(bracket.finalized_at)}.</strong></p> : <div className="worlds-finalize-form"><p>Finalize only after all {bracket.field_size - 1} played matches match the official bracket.</p><label>Official final bracket URL<input type="url" value={finalSourceUrl} onChange={(event) => setFinalSourceUrl(event.target.value)} /></label><label>Type <strong>FINALIZE OFFICIAL BRACKET</strong><input value={finalConfirmation} onChange={(event) => setFinalConfirmation(event.target.value)} /></label><button className="primary-button danger-button" disabled={busy || !allResults || !finalSourceUrl || finalConfirmation !== "FINALIZE OFFICIAL BRACKET"} onClick={() => mutate({ action: "finalize", official_source_url: finalSourceUrl, confirmation_text: finalConfirmation }, "The reviewed bracket results are final.")}>Finalize bracket</button></div>}</details>}
