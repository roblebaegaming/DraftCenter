...
function incidentTiming(event) {
  if ((event?.occurrence_count || 1) === 1) return `${when(event?.occurred_at)} · ${event?.actor || "Unknown user"}`;
  return `${event.occurrence_count} reports · first ${when(event.first_occurred_at)} · latest ${when(event.last_occurred_at)} · latest report by ${event.actor || "Unknown user"}`;
}
...
export default function OperationsDashboard() {
  ...
  function reminderMessage(league) { return `Subject: ${league.commissioner_reminder?.subject || ""}\n\n${league.commissioner_reminder?.text || ""}`; }
  ...
  async function copyReminder() { if (!reminderLeague) return; try { await navigator.clipboard.writeText(reminderMessage(reminderLeague)); setCopyStatus("Reminder copied. Nothing was sent."); } catch { setCopyStatus("Copy was blocked. Select the message below and copy it manually. Nothing was sent."); } }
  ...
  return <main className="operations-shell">
    ...
    {(data.operational_failures || []).length > 0 && <details className="operations-error-feed"><summary><strong>System failures</strong> · last 30 days</summary><p>Unexpected incidents that may require investigation or a code, configuration, or provider fix. Repeated matching reports within five minutes are grouped together.</p>{data.operational_failures.map((event)=><article key={event.id}><span className="eyebrow">{event.kind.replaceAll("_"," ")}</span><h3>{event.league_name}</h3><p>{event.message}</p><small>{incidentTiming(event)}</small></article>)}</details>}
    {(data.resolved_operational_incidents || []).length > 0 && <details className="operations-resolved-feed"><summary><strong>Resolved incidents</strong> · last 30 days</summary><p>Historical failures recorded before a verified fix. A matching report after its fix is treated as a new system failure.</p>{data.resolved_operational_incidents.map((event)=><article key={event.id}><span className="eyebrow">{event.kind.replaceAll("_"," ")}</span><h3>{event.league_name}</h3><p>{event.message}</p><small>{incidentTiming(event)}</small><a href={event.resolution_url} target="_blank" rel="noreferrer">{event.resolution_label}</a></article>)}</details>}
    {(data.operational_rejections || []).length > 0 && <details className="operations-rejection-feed"><summary><strong>Expected safety rejections</strong> · last 30 days</summary><p>Permission checks, stale-session protection, duplicate picks, and other server safeguards working as intended. Repeated matching reports within five minutes are grouped together.</p>{data.operational_rejections.map((event)=><article key={event.id}><span className="eyebrow">{event.kind.replaceAll("_"," ")}</span><h3>{event.league_name}</h3><p>{event.message}</p><small>{incidentTiming(event)}</small></article>)}</details>}
