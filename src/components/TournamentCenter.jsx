...
function TournamentDetail({ detail, user, busy, setSelectedId, act, sheet, setSheet, saveSheet }) {
  ...
  const canJudge = organizer || staffMember?.role === "judge";
...
function OrganizerJudgeDesk({ event, organizer, entrants, pairings, disputes, penalties, names, busy, act }) {
  ...
  return <section className="tournament-panel organizer-judge-desk">
    ...
    {organizer&&<><div className="tournament-staff-controls"><label>Event visibility<select value={event.visibility||"private"} onChange={(e)=>act("set_tournament_visibility",{p_tournament_id:event.id,p_visibility:e.target.value},"Event visibility updated.")}><option value="private">Private</option><option value="public">Public</option></select></label><label>Judge username<input value={staffUsername} onChange={(e)=>setStaffUsername(e.target.value)} placeholder="username"/></label><button className="secondary-button" disabled={busy||!staffUsername.trim()} onClick={()=>act("appoint_tournament_staff",{p_tournament_id:event.id,p_username:staffUsername,p_role:staffRole},"Judge appointed.")}>Appoint judge</button></div><div className="league-tool-compact-actions"><button className="quiet-button" onClick={createPrivateInvite}>Copy private registration invite</button>{inviteLink&&<input readOnly value={inviteLink}/>}</div></>}
