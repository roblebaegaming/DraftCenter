...
export default function NuzlockeLab() {
  ...
  return <main className="nuzlocke-shell"><header className="nuzlocke-hero"><a href="/?view=dashboard" className="quiet-button">← DraftCenter home</a><span className="eyebrow">NUZLOCKE DRAFT</span><h1>Build a Nuzlocke Team</h1><p>Generate a repeatable team from verified, game-specific encounters. DraftCenter never changes your leagues, drafts, or rosters from this page.</p></header>
    <div className="nuzlocke-layout"><form className="nuzlocke-controls" onSubmit={generate}><h2>Run rules</h2>
      ...
      <div className="nuzlocke-run-code-field"><div className="nuzlocke-pair"><label>Team code<input value={seed} maxLength={80} onChange={(event) => setSeed(event.target.value)} /></label><button type="button" className="quiet-button" onClick={() => setSeed(newSeed())}>New team code</button></div><small>Generated automatically. Keep it only if you want to recreate or share this exact team.</small></div>
      ...
      <button className="primary-button" disabled={loading || !game || !seed}>{loading ? "Building…" : "Build a Nuzlocke Team"}</button>{message && <p className="hub-message" role="status">{message}</p>}
    </form>
    <section className="nuzlocke-output"><div className="section-heading"><div><span className="eyebrow">RUN CARD</span><h2>{result?.game?.display_name || "Your encounters"}</h2></div>{shareUrl && <button className="quiet-button" type="button" onClick={() => navigator.clipboard.writeText(shareUrl)}>Copy team link</button>}</div>
      ...
      <div className="nuzlocke-team">{result?.team?.map((entry, index) => {
        ...
        return <article key={`${entry.area_key}-${entry.pokemon_id}`}><span className="nuzlocke-number">{index + 1}</span>{entry.artwork_url && <img src={entry.artwork_url} alt="" />}<div><h3><a href={`/pokemon/${profileSlug}`}>{entry.pokemon_name}{entry.form_name ? ` (${entry.form_name})` : ""}</a></h3><strong>{entry.area_name}</strong><p>{entry.method === "starter" ? "Starter Pokémon" : <>{entry.encounter_pokemon_name ? `Catch ${entry.encounter_pokemon_name}${entry.encounter_form_name ? ` (${entry.encounter_form_name})` : ""} · ` : ""}{pretty(entry.method)} · Lv. {entry.min_level ?? "?"}{entry.max_level && entry.max_level !== entry.min_level ? `–${entry.max_level}` : ""}</>}</p>{entry.conditions?.length ? <small>{entry.conditions.map(pretty).join(", ")}</small> : <small>No special conditions</small>}</div></article>;
      })}</div>
    </section></div></main>;
}
