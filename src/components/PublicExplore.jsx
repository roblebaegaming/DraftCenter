...
export default function PublicExplore() {
  ...
  return <main className="explore-shell">
    ...
    {data && <div className="explore-grid">
      ...
      <Ranking onSelectPokemon={setSelectedPokemon} title="Highest public-league win rates" items={trends?.win_rates} empty="Win rates appear after Pokémon teams complete at least two confirmed public matches." render={(item) => <span><strong>{item.pokemon}</strong><small>{item.win_rate}% · {item.wins}-{item.games - item.wins} across {item.games} matches</small></span>} />
      ...
      <Ranking onSelectPokemon={setSelectedPokemon} title="Community ADP" items={data.adp} empty="ADP begins to form after a snake draft is completed." render={(item) => <span><strong>{item.pokemon}</strong><small>ADP {item.average_pick} · selected in {item.drafts} of {item.eligible_drafts || item.drafts} eligible draft{(item.eligible_drafts || item.drafts) === 1 ? "" : "s"}</small></span>} />
    </div>}
  </main>;
}
