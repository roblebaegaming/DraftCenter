...
export async function GET(request) {
  ...
  return NextResponse.json({ current, audit: audit || [] });
}
...
export async function POST(request) {
  const supabase = createAdminClient(); const auth = await authenticateUser(request, supabase);
  if (auth.error) return NextResponse.json({ error: auth.error }, { status: auth.status });
  const body = await request.json(); const leagueId = body.league_id; const hours = Number(body.hours || 24);
  if (!leagueId || !allowedHours.has(hours)) return NextResponse.json({ error: "Choose a valid support duration." }, { status: 400 });
  ...
  await supabase.from("league_support_grants").update({ revoked_at: new Date().toISOString(), revoked_by: auth.user.id }).eq("league_id", leagueId).is("revoked_at", null);
  const expiresAt = new Date(Date.now() + hours * 3600000).toISOString();
  const { data: grant, error } = await supabase.from("league_support_grants").insert({ league_id: leagueId, support_user_id: supportUser.id, approved_by: auth.user.id, permission: "read_only", expires_at: expiresAt }).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  await supabase.from("league_support_audit_log").insert({ league_id: leagueId, grant_id: grant.id, actor_user_id: auth.user.id, action: "approved", details: { permission: "read_only", hours, expires_at: expiresAt } });
