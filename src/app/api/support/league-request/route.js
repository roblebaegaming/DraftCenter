...
import { readBoundedJson, safeDiagnosticMessage, safeFailure, safeStoredFailure, UUID_PATTERN } from "../../../../lib/apiSecurity";
...
export async function POST(request) {
  ...
  const parsed = await readBoundedJson(request, { maxBytes: 8 * 1024, maxDepth: 4, maxEntries: 30, maxArrayLength: 10, maxStringLength: 2_000 });
  if (parsed.error) return NextResponse.json({ error: parsed.error }, { status: parsed.status });
  const body = parsed.data; const leagueId = body.league_id; const category = String(body.category || ""); const message = String(body.message || "").trim();
  if (!UUID_PATTERN.test(String(leagueId || "")) || !categories.has(category) || message.length < 10 || message.length > 2000) return NextResponse.json({ error: "Choose a category and enter 10–2,000 characters." }, { status: 400 });
  ...
  if (includeDiagnostics && body.context && typeof body.context === "object") for (const [key,value] of Object.entries(body.context)) if (allowedContext.has(key)) context[key] = typeof value === "string" ? (key === "last_error" ? safeDiagnosticMessage(value) : value.slice(0,500)) : value;
  ...
  if (error) return safeFailure(error, "The support request could not be saved.", { context: "support-request-save" });
  ...
  } catch (notifyError) { notificationError = safeStoredFailure("Owner notification could not be delivered."); await supabase.from("league_support_requests").update({ notification_error: notificationError }).eq("id", supportRequest.id); }
