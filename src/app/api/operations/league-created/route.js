...
import { bearerToken, readBoundedJson, safeStoredFailure, UUID_PATTERN } from "../../../../lib/apiSecurity";

export const runtime = "nodejs";
export async function POST(request) {
  const token = bearerToken(request); const parsed = await readBoundedJson(request, { maxBytes: 2048, maxDepth: 2, maxEntries: 5, maxArrayLength: 1, maxStringLength: 100 });
  if (parsed.error) return NextResponse.json({ error: parsed.error }, { status: parsed.status });
  const body = parsed.data;
  if (!token || !UUID_PATTERN.test(String(body.league_id || ""))) return NextResponse.json({ error: "League and session are required." }, { status: 400 });
  ...
  if (!await consumeUserRateLimit(supabase, "league-created-notification", `${user.id}:${body.league_id}`, 5, 3600)) return NextResponse.json({ error: "League notifications were requested too many times. Try again later." }, { status: 429 });
  ...
  if (user.email) {
    ...
    } catch(onboardingError) { await supabase.from("league_onboarding_deliveries").update({failed_at:new Date().toISOString(),last_error:safeStoredFailure("Commissioner onboarding email could not be delivered.")}).eq("league_id",league.id); }
  }
  ...
  for (const recipient of ownerEmails()) {
    ...
    catch (error) { await supabase.from("owner_notification_deliveries").update({ failed_at: new Date().toISOString(), last_error: safeStoredFailure("Owner notification could not be delivered.") }).eq("dedupe_key", dedupe); }
  }
