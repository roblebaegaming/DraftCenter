...
export async function POST(request) {
  try {
    ...
    const catalogClient = createPublicServerClient();
    if (!catalogClient) throw new Error("DraftCenter public catalog access is not configured.");
    const { data: game, error: gameError } = await catalogClient.from("pokemon_games").select("game_key,display_name").eq("game_key", body.game).eq("encounter_status", "verified").maybeSingle();
    ...
    return Response.json({ game, seed, ...result }, { headers: { "Cache-Control": "private, no-store" } });
  } catch (error) {
