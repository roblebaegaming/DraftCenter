...
test("release migrations use one production number each", () => {
  ...
  assert.ok(migrations.includes("414-separate-pokedex-and-encounter-verification.sql"));
  assert.ok(migrations.includes("415-import-pokemon-legends-za-pokedex.sql"));
});
...
test("integrated quick links expose each released feature once", () => {
  ...
  for (const path of ["/team-lab", "/nuzlocke", "/tournaments", "/calendar", "/predictions", "/trainer-dex", "/pokedex-tracker", "/operations"]) {
  ...
  assert.match(links, /className="site-primary-links"[\s\S]*?href="\/predictions"[^>]*>Live Predictions<\/a>/);
  assert.doesNotMatch(links.slice(links.indexOf('<nav className={`site-quick-links')), /href="\/predictions"/);
  const predictions = source("src/app/predictions/page.js");
  assert.match(predictions, /href="\/worlds\/2026"/);
