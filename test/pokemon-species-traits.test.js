...
test("the pinned species-trait catalog contains every PokeAPI profile and official category label", () => {
  ...
  assert.equal(Object.keys(catalog.profile_species).length, catalog.pokemon_count);
  assert.equal(Object.keys(catalog.species).length, catalog.species_count);
  assert.equal(catalog.colors.length, 10);
  assert.equal(catalog.shapes.length, 14);
  assert.equal(catalog.egg_groups.length, 15);
  assert.equal(new Set(catalog.colors.map((item) => item.id)).size, 10);
  ...
  assert.ok(catalog.colors.every((item) => item.id && item.label));
  ...
  assert.equal(catalog.profile_species.bulbasaur, 1);
  assert.deepEqual(catalog.species["1"], { name: "bulbasaur", color: "green", shape: "quadruped", egg_groups: ["monster", "plant"] });
  assert.equal(catalog.species["25"].color, "yellow");
