...
test("reviewed Generation IX catalogs support DLC, opt-in mechanics, starters, and form-aware final evolutions",()=>{
  for(const game of ["scarlet","violet"]){
    ...
    const finals=generateNuzlockeTeam(catalog.encounters,{...options,seed:`${game}-finals`,includeStarter:false,finalEvolutionOnly:true,evolutionCatalog});assert.equal(finals.complete,true);assert.ok(finals.team.every((row)=>row.is_final_evolution));assert.deepEqual(new Set(evolutionCatalog.evolutions.map((row)=>`${row.pokemon_id}|${row.form_name||""}`)),new Set(catalog.encounters.map((row)=>`${row.pokemon_id}|${row.form_name||""}`)));
  }
