...
test("reviewed Generation IX catalogs support DLC, opt-in mechanics, starters, and form-aware final evolutions",()=>{
  for(const game of ["scarlet","violet"]){
    const catalog=JSON.parse(fs.readFileSync(new URL(`../data/nuzlocke/pokemon-${game}.pokeapi-5064f1d72746b3a6a931616dae3fb6445c556d4f.json`,import.meta.url),"utf8"));
    const evolutionCatalog=JSON.parse(fs.readFileSync(new URL(`../data/nuzlocke/pokemon-${game}-evolutions.pokeapi-5064f1d72746b3a6a931616dae3fb6445c556d4f.json`,import.meta.url),"utf8"));
    const options={seed:`${game}-review`,teamSize:12,mode:"route-random",weighting:"authentic",familyClause:true,excludeLegendaries:true,includeStarter:true,starters:catalog.game.starters,conditionGroups:catalog.game.condition_groups};
    const result=generateNuzlockeTeam(catalog.encounters,options);assert.equal(result.complete,true);assert.ok(catalog.game.starters.some((row)=>row.pokemon_id===result.team[0].pokemon_id));assert.equal(new Set(result.team.map((row)=>row.species_family)).size,12);assert.ok(result.team.every((row)=>!(row.conditions||[]).some((condition)=>["content-teal-mask","content-indigo-disk","tera-raid-encounter","union-circle-required","limited-time-event","league-club-trade"].includes(condition))));assert.deepEqual(result,generateNuzlockeTeam(catalog.encounters,options));
    const expanded={...options,seed:`${game}-expanded`,includeStarter:false,conditionSelections:{content:"indigo-disk","tera-raids":"on","group-quests":"on","limited-events":"on","league-club-trades":"on"}};const withOptions=generateNuzlockeTeam(catalog.encounters,expanded);assert.equal(withOptions.complete,true);assert.equal(withOptions.conditionSelections.content,"indigo-disk");assert.equal(withOptions.conditionSelections["tera-raids"],"on");assert.deepEqual(withOptions,generateNuzlockeTeam(catalog.encounters,expanded));
    const finals=generateNuzlockeTeam(catalog.encounters,{...options,seed:`${game}-finals`,finalEvolutionOnly:true,evolutionCatalog});assert.equal(finals.complete,true);assert.ok(finals.team.every((row)=>row.is_final_evolution));assert.ok([908,911,914].includes(finals.team[0].pokemon_id));assert.equal(finals.team[0].method,"starter");assert.deepEqual(new Set(evolutionCatalog.evolutions.map((row)=>`${row.pokemon_id}|${row.form_name||""}`)),new Set([...catalog.encounters,...catalog.game.starters].map((row)=>`${row.pokemon_id}|${row.form_name||""}`)));
  }
  const evolutions=JSON.parse(fs.readFileSync(new URL("../data/nuzlocke/pokemon-scarlet-evolutions.pokeapi-5064f1d72746b3a6a931616dae3fb6445c556d4f.json",import.meta.url),"utf8")).evolutions;
  assert.deepEqual(evolutions.find((row)=>row.pokemon_id===217&&row.form_name==="").final_evolutions.map((row)=>row.pokemon_id),[217]);
  assert.deepEqual(evolutions.find((row)=>row.pokemon_id===10151&&row.form_name==="Own Tempo").final_evolutions.map((row)=>row.pokemon_id),[10152]);
  assert.deepEqual(evolutions.find((row)=>row.pokemon_id===1012&&row.form_name==="Artisan Form").final_evolutions.map((row)=>[row.pokemon_id,row.form_name]),[[1013,"Masterpiece Form"]]);
});
