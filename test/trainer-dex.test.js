...
test("backfill cannot award shinies and an undone draft removes its source event",()=>{const sql=source("supabase/261-trainer-dex-and-shiny-discoveries.sql");assert.equal((sql.match(/record_trainer_dex_event\([^;]+,false\)/g)||[]).length,4);assert.match(sql,/if tg_op='DELETE'/);assert.match(sql,/delete from public\.trainer_dex_events where source_type='draft' and source_id=old\.id::text/);assert.match(sql,/after insert or delete on public\.draft_picks/);});
