...
const blueImported=fs.readFileSync(new URL("../supabase/265-import-pokemon-blue-encounter-catalog.sql",import.meta.url),"utf8");
const blueVerified=fs.readFileSync(new URL("../supabase/266-verify-pokemon-blue-encounter-catalog.sql",import.meta.url),"utf8");
...
const blueArtifact=JSON.parse(fs.readFileSync(new URL("../data/nuzlocke/pokemon-blue.pokeapi-5064f1d72746b3a6a931616dae3fb6445c556d4f.json",import.meta.url),"utf8"));
const blueEvolutionArtifact=JSON.parse(fs.readFileSync(new URL("../data/nuzlocke/pokemon-blue-evolutions.pokeapi-5064f1d72746b3a6a931616dae3fb6445c556d4f.json",import.meta.url),"utf8"));
...
test("Blue imports pending and verifies only its exact version-specific snapshot",()=>{ assert.match(blueImported,/encounter_status\) values \('blue'[\s\S]+,'pending'\)/); assert.doesNotMatch(blueImported,/encounter_status='verified'/); assert.match(blueImported,/on conflict\(game_key,source_encounter_id\)/); assert.match(blueVerified,/count\(\*\)[\s\S]+<> 151/); assert.match(blueVerified,/count\(\*\)[\s\S]+<> 74/); assert.match(blueVerified,/count\(\*\)[\s\S]+<> 891/); assert.match(blueVerified,/kanto-route-22-main-area'[\s\S]+pokemon_id=29[\s\S]+chance=20/); assert.match(blueVerified,/pokemon-mansion-b1f'[\s\S]+pokemon_id=126[\s\S]+chance=4/); assert.match(blueVerified,/where game_key='blue'[\s\S]+encounter_status='pending'/); });
...
test("reviewed Blue artifacts are pinned, complete, version-specific, and evolution-safe",()=>{
  ...
  assert.equal(new Set(blueArtifact.encounters.map((row)=>row.source_encounter_id)).size,891);
  const route22=blueArtifact.encounters.filter((row)=>row.area_key==="kanto-route-22-main-area"&&row.method==="walk");
  assert.ok(route22.some((row)=>row.pokemon_id===29&&row.min_level===3&&row.chance===20));
  assert.ok(!route22.some((row)=>row.pokemon_id===32&&row.min_level===3&&row.chance===20));
  const mansion=blueArtifact.encounters.filter((row)=>row.area_key==="pokemon-mansion-b1f"&&row.method==="walk");
  assert.ok(mansion.some((row)=>row.pokemon_id===126&&row.min_level===38&&row.chance===4));
  assert.ok(!mansion.some((row)=>row.pokemon_id===58));
  assert.equal(blueEvolutionArtifact.game_key,"blue");
  assert.equal(blueEvolutionArtifact.source_commit,"5064f1d72746b3a6a931616dae3fb6445c556d4f");
  assert.deepEqual(new Set(blueEvolutionArtifact.evolutions.map((row)=>row.pokemon_id)),new Set(blueArtifact.encounters.map((row)=>row.pokemon_id)));
  assert.ok(blueEvolutionArtifact.evolutions.every((row)=>row.final_evolutions.length));
});
...
test("final evolution mode is shareable and the UI explains run codes and both random styles",()=>{ assert.match(lab,/params\.get\("evolutions"\) === "final"/); assert.match(lab,/url\.searchParams\.set\("evolutions", "final"\)/); assert.match(lab,/finalEvolutionOnly/); assert.match(lab,/Catch \$\{entry\.encounter_pokemon_name\}/); assert.match(lab,/Run code/); assert.match(lab,/Route-first random/); assert.match(lab,/Encounter-pool random/); assert.match(lab,/locations with more eligible entries can appear more often/); });
