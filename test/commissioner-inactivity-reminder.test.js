...
import { buildCommissionerInactivityReminder, commissionerInactivityEligibility } from "../src/lib/commissionerInactivityReminder.js";
...
test("the reminder fails closed when meaningful setup activity exists", () => {
  const cases = [
    ...
    [{ league: { ...base.league, created_at: "2026-07-19T18:00:00.000Z" } }, "too_old"],
  ];
...
test("automation is cron-only, disabled by default, deduplicated, and rechecked before delivery", () => {
  ...
  const migration = fs.readFileSync(new URL("../supabase/migrations/20260819011925_440_commissioner_inactivity_reminders.sql", import.meta.url), "utf8");
  ...
  assert.match(cronRoute, /ageDays < 9/);
  ...
  assert.match(dispatchRoute, /maximumAgeDays: 9/);
