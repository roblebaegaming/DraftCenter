-- Publish the roster rule that actually governs each public league.
-- Fixed-size snake drafts use settings.rosterSize; auction and budget-snake
-- drafts continue to use settings.rosterMin/settings.rosterMax.
...
as $$
  ...
  from (
    select l.id, l.name, l.slug, l.description, l.image_url, l.season_label, l.status,
      ...
      case
        when coalesce(nullif(s.state #>> '{settings,draftType}', ''), 'snake') = 'snake'
          and not coalesce((s.state #>> '{settings,snakeBudgetEnabled}')::boolean, false)
          then coalesce(
            nullif(s.state #>> '{settings,rosterSize}', '')::integer,
            nullif(s.state #>> '{settings,rosterMin}', '')::integer
          )
        else nullif(s.state #>> '{settings,rosterMin}', '')::integer
      end as roster_min,
      case
        when coalesce(nullif(s.state #>> '{settings,draftType}', ''), 'snake') = 'snake'
          and not coalesce((s.state #>> '{settings,snakeBudgetEnabled}')::boolean, false)
          then coalesce(
            nullif(s.state #>> '{settings,rosterSize}', '')::integer,
            nullif(s.state #>> '{settings,rosterMax}', '')::integer
          )
        else nullif(s.state #>> '{settings,rosterMax}', '')::integer
      end as roster_max,
