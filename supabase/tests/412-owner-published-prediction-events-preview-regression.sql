...
begin
  select
    has_function_privilege('anon', 'public.list_prediction_bracket_events()', 'execute')
    and has_function_privilege('authenticated', 'public.list_prediction_bracket_events()', 'execute')
  ...
  if (v_created ->> 'public_path') <> ('/predictions/' || v_event_id) then
    raise exception 'The event creator did not return the stable public path.';
  end if;

  select public.list_prediction_bracket_events() into v_directory;
  select not exists (
  ...
  select exists (
    ...
    where item ->> 'event_id' = v_event_id
      and (item ->> 'public_path') = ('/predictions/' || v_event_id)
