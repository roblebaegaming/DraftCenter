-- Run only after migration 377 on an isolated Supabase Preview branch.
...
begin
  if not exists (
    ...
    raise exception 'The reviewed Pokémon GO Pick 10 event is not open.';
  end if;

  if (select count(*) from public.worlds_pick_competitors where event_id = '2026-pokemon-go') <> 369 then
    raise exception 'Expected the 369-person official Pokémon GO qualifier pool.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Pokémon GO result polling must remain disabled and unconfigured.';
  end if;
  ...
  if not has_function_privilege('anon', 'public.get_worlds_pick_hub(text)', 'execute')
    ...
    raise exception 'Pokémon GO Pick 10 RPC grants are incomplete.';
  end if;
  ...
  if cardinality(v_picks) <> 10 then
    raise exception 'Could not build a complete Pokémon GO Pick 10 fixture.';
  end if;
  ...
  perform public.save_worlds_pick_entry('2026-pokemon-go', v_picks, v_picks[1]);

  select public.get_worlds_pick_hub('2026-pokemon-go') into v_hub;
  if (v_hub ->> 'entry_count')::integer <> 1
    ...
    raise exception 'The saved Pokémon GO entry or open event state did not round-trip.';
  end if;

  begin
    perform public.save_worlds_pick_entry('2026-pokemon-go', v_picks[1:9], v_picks[1]);
    raise exception 'An incomplete Pokémon GO entry unexpectedly saved.';
  ...
  if v_hub -> 'my_entry' <> 'null'::jsonb
    ...
    raise exception 'Another member could see a private Pokémon GO entry before lock.';
  end if;
  ...
  if exists (select 1 from public.worlds_pick_entries where user_id in (v_owner, v_observer)) then
    raise exception 'Temporary Pokémon GO Preview entries did not clean up.';
  end if;
end;
$$;
