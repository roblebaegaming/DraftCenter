-- Private Team Lab held items and ability/item turn reveals.
-- Existing rows remain valid; the new JSON fields are optional until a coach saves them.
  ...
  select coalesce((
    ...
    and not exists (
      ...
      where jsonb_typeof(entry) is distinct from 'object'
         ...
         or (entry ? 'item' and jsonb_typeof(entry -> 'item') is distinct from 'string')
         or char_length(coalesce(entry ->> 'item', '')) > 100
  ...
  select coalesce((p_log is not null
    ...
    and not exists (
      ...
      where jsonb_typeof(entry) is distinct from 'object'
        ...
        or entry ->> 'kind' not in ('move', 'ability', 'item', 'switch', 'faint', 'note')
        ...
        or (entry ? 'detail' and jsonb_typeof(entry -> 'detail') is distinct from 'string')
        or char_length(coalesce(entry ->> 'detail', '')) > 100
        ...
        or (entry ->> 'kind' in ('ability', 'item') and char_length(btrim(coalesce(entry ->> 'detail', ''))) not between 1 and 100)
        or (entry ->> 'kind' not in ('ability', 'item') and coalesce(entry ->> 'detail', '') <> '')
...
comment on function public.is_valid_team_lab_opponent_sets(jsonb, jsonb) is
  'Validates bounded opponent scouting, including an optional held item, aligned to the private opponent roster.';
comment on function public.is_valid_team_lab_turn_log(jsonb, jsonb, jsonb) is
  'Validates bounded move, ability, item, switch, faint, and note events against the private Battle Mode rosters.';
comment on column public.team_lab_matchups.battle_report is
  'Private owner-only Battle Mode state, including held-item scouting and a bounded turn timeline. Exported and restored only through owner RPCs.';
...
begin
  if not exists (
    ...
    raise exception 'Team Lab reveal recording requires forced row-level security.';
  end if;
  if has_table_privilege('anon', 'public.team_lab_matchups', 'select')
    ...
    raise exception 'Team Lab reveal recording must remain RPC-only.';
  end if;
  ...
  if has_function_privilege('anon', 'public.is_valid_team_lab_opponent_sets(jsonb,jsonb)', 'execute')
     or has_function_privilege('authenticated', 'public.is_valid_team_lab_opponent_sets(jsonb,jsonb)', 'execute')
     or has_function_privilege('anon', 'public.is_valid_team_lab_turn_log(jsonb,jsonb,jsonb)', 'execute')
     or has_function_privilege('authenticated', 'public.is_valid_team_lab_turn_log(jsonb,jsonb,jsonb)', 'execute')
