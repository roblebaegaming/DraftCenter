-- Migration 431: Pokédex completeness and encounter completeness are independent.
-- A reviewed Pokédex may be browsed and tracked without exposing incomplete
-- encounter data to Nuzlocke generation.
...
create policy pokemon_game_pokedex_verified_read on public.pokemon_game_pokedex_entries
  for select to anon, authenticated
  using (exists (
    select 1 from public.pokemon_games game
    where game.game_key = pokemon_game_pokedex_entries.game_key
      and game.pokedex_status = 'verified'
  ));

-- Location and encounter policies intentionally remain encounter-status only.
...
as $$
  ...
  home_supplement as (
    select supplement.pokemon_id, supplement.pokemon_name,
      supplement.pokemon_id as dex_number, 'national'::text as pokedex_key,
...
begin
  ...
  if p_catalog_key = 'home' then
    ...
    select display_name into v_catalog_name from public.pokemon_games
...
grant execute on function public.create_my_pokedex_tracker(text, text, boolean) to authenticated, service_role;

do $$
begin
  if (select count(*) from public.pokedex_tracker_catalog('home')) <> 1025 then
