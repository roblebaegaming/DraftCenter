-- Create the intermediate matchup object when a league receives its first
-- prediction for that matchup. Migrations 366 and 367 already ran in Preview,
-- so this correction is forward-only.
...
begin
  ...
  v_state := jsonb_set(
    v_state,
    '{predictions}',
    coalesce(v_state -> 'predictions', '{}'::jsonb) || jsonb_build_object(
      v_key,
      coalesce(v_state #> array['predictions', v_key], '{}'::jsonb)
        || jsonb_build_object(v_name, v_existing || v_safe_patch)
    ),
    true
  );
