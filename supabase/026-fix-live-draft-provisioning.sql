-- DraftCenter milestone 7: fix live-draft provisioning for scheduled and
-- immediate snake drafts. The previous routine tried to assign directly into
-- a UUID array in a RETURNING clause; PostgreSQL treated that target as one
-- UUID and raised "cannot subscript type uuid".
...
begin
  ...
  loop
    ...
    -- Append after the insert/update. This preserves the prototype's numeric
    -- team index without trying to subscript a scalar UUID value.
  ...
  for v_pokemon in select value from jsonb_array_elements(p_pokemon) loop
    ...
    values (
      v_pokemon ->> 'id',
      coalesce(v_pokemon ->> 'name', v_pokemon ->> 'id'),
    ...
    on conflict (league_id, source_key) where source_key is not null do update
      set cost = excluded.cost, is_allowed = true, is_drafted = false;
  end loop;

  update public.leagues
  set settings = coalesce(settings, '{}'::jsonb) || coalesce(p_settings, '{}'::jsonb) || jsonb_build_object('rosterMax', p_rounds),
      updated_at = now()
  ...
  if array_length(v_order_ids, 1) <> jsonb_array_length(p_teams)
