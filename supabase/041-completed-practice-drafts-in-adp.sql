-- Let completed practice/test snake drafts build DraftCenter's anonymous ADP.
-- Practice leagues remain excluded from coach records, standings, win rates,
-- and other competitive statistics. Incomplete practice drafts do not count.
...
as $$
  with current_poll as (
    ...
    select pc.display_name as pokemon,
      ...
      round(avg(dp.pick_number)::numeric, 1) as average_pick
    ...
    where ds.mode = 'snake'
      and (
        (
          ...
          and ds.status in ('active', 'paused', 'complete')
        )
        or (
          l.is_practice
  ...
  select jsonb_build_object(
    ...
    'leagues', coalesce((
      select jsonb_agg(to_jsonb(public_leagues)) from public_leagues
    ), '[]'::jsonb),
    'popularity', coalesce((
      select jsonb_agg(jsonb_build_object('pokemon', pokemon, 'favorites', total))
