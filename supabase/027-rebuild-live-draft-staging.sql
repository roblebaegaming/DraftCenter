-- DraftCenter milestone 8: make the first live-draft start retry-safe.
--
-- A failed earlier start could leave behind partially created relational
-- teams. The next attempt then saw those stale rows in addition to the
-- commissioner’s current setup teams and rejected the otherwise-valid order.
-- Before a draft session exists, those rows are only staging data, so rebuild
-- them from the saved setup every time a commissioner starts the draft.
...
begin
  ...
  if jsonb_array_length(coalesce(p_pokemon, '[]'::jsonb)) = 0 then
    raise exception 'No eligible Pokémon were supplied.';
  end if;
  ...
  -- Remove only abandoned bootstrap data from a prior failed start. A real
  -- draft always has a session and is protected by the check above.
  ...
  loop
    ...
    if v_owner_name is not null then
      ...
      where lower(username) = lower(v_owner_name) or lower(display_name) = lower(v_owner_name)
      limit 1;
      if v_owner_id is not null then
        insert into public.league_memberships (league_id, user_id, role)
        values (p_league_id, v_owner_id, 'coach')
        on conflict (league_id, user_id) do update
          set role = case when public.league_memberships.role = 'viewer' then 'coach' else public.league_memberships.role end
  ...
  for v_pokemon in select value from jsonb_array_elements(p_pokemon) loop
    if nullif(v_pokemon ->> 'id', '') is null then
      raise exception 'Every Pokémon needs a stable source id.';
    end if;
  ...
  set settings = coalesce(settings, '{}'::jsonb) || coalesce(p_settings, '{}'::jsonb) || jsonb_build_object('rosterMax', p_rounds), updated_at = now()
  ...
  if coalesce(array_length(v_order_ids, 1), 0) <> v_team_count
     or exists (select 1 from unnest(v_order_ids) as id where id is null) then
    raise exception 'The draft order could not be built. Refresh Setup and try again.';
  end if;
