-- Restore the ordered-claim schema that the client and priority-round
-- resolver expect. The live database reached migration 226 without the
-- table/function portion of migration 216.
...
revoke all on function public.list_private_free_agent_claims(uuid)
  from public, anon, authenticated;
revoke all on function public.move_private_free_agent_claim(
  uuid, uuid, integer
)
  from public, anon, authenticated;
grant execute on function public.list_private_free_agent_claims(uuid)
  to authenticated;
grant execute on function public.move_private_free_agent_claim(
  uuid, uuid, integer
)
  to authenticated;
