...
if (!["red", "blue", "yellow", "gold", "silver", "crystal", "ruby", "sapphire", "emerald", "firered", "leafgreen", "diamond", "pearl", "platinum", "heartgold", "soulsilver", "black", "white", "black-2", "white-2", "x", "y", "omega-ruby", "alpha-sapphire", "sun", "moon", "ultra-sun", "ultra-moon", "lets-go-pikachu", "lets-go-eevee", "sword", "shield", "brilliant-diamond", "shining-pearl", "legends-arceus", "scarlet", "violet"].includes(game)) throw new Error("The reviewed migration builder accepts only supported Generation I–IX games.");
...
const independentSources = Number(payload.game.generation) === 9 ? "PKHeX, pkNX, and a pinned version-exclusive source review" : Number(payload.game.generation) === 8 ? (game.includes("diamond") || game.includes("pearl") ? "PKHeX and BDSP-Randomizers" : "PKHeX and pkNX") : Number(payload.game.generation) === 7 ? (game.startsWith("lets-go-") ? "PKHeX" : "PKHeX and pk3DS") : Number(payload.game.generation) === 6 ? "Veekun, PKHeX, and pk3DS" : Number(payload.game.generation) === 5 ? "Veekun and PKHeX" : `Veekun and pret/${pretRepository}`;
