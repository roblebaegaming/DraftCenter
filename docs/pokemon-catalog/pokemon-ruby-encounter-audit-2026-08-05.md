# Pokémon Ruby encounter audit — 2026-08-05

Status: independently audited locally; forward-only migrations 276–277 are prepared but unapplied.
...
- 202 Hoenn Pokédex rows, 103 encounter areas, 1,530 encounter entries, and 129 obtainable encounter profiles.
...
- Final-evolution data is capped at the 386 species available by Generation III, so postgame or externally obtainable profiles outside the 202-entry Hoenn Dex remain evolution-safe without acquiring later-generation evolutions.

The automated audit found no Veekun tuples missing after documented location/method normalization and exactly 52 independently sourced additions. Ruby-only checks matched Seedot on Route 102, Solrock in Meteor Falls, and Zangoose on Route 114 in the pinned pokeruby tables.
