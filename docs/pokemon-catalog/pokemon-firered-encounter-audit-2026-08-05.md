# Pokémon FireRed encounter audit — 2026-08-05

Status: independently audited locally; forward-only migrations 282–283 are prepared but unapplied.
...
- The roaming beast is automatically matched to the included seeded starter: Bulbasaur→Entei, Charmander→Suicune, and Squirtle→Raikou. Manual starter-state filtering remains available when a starter is not included.
- The normal Zubat Altering Cave table is the default; all eight event tables remain explicit choices under the same encounter location, so mutually exclusive states never count as separate catches. Post-Elite-Four-rematch state is independently filterable.
...
The automated audit found no Veekun tuples missing after documented normalization and exactly 30 independently sourced additions. FireRed checks matched Kakuna in Viridian Forest, Growlithe in Pokémon Mansion, and Skarmory in Sevault Canyon in the pinned pokefirered tables.
