# Pokémon SoulSilver encounter audit — 2026-08-05

Status: independently audited locally; forward-only migrations 294–295 are prepared but unapplied.
...
SoulSilver-specific sentinels include morning Ledyba on Route 30, Vulpix on Route 36, and level-50 Groudon in the Embedded Tower. The pinned disassembly independently exposes the Growlithe/Vulpix and Spinarak/Ledyba version branches plus the three Headbutt table classes. SoulSilver has 323 unique tuples absent from HeartGold; HeartGold has 320 absent from SoulSilver.
...
`npm run catalog:audit:soulsilver` passes. Migration 294 imports the catalog as `pending`; migration 295 verifies exact counts, metadata, resolvable areas, and SoulSilver sentinels before publishing it as `verified`. Neither migration has been applied to Preview or production.
