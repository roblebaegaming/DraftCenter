# Pokémon Pearl encounter audit — 2026-08-05

Status: independently audited locally; forward-only migrations 288–289 are prepared but unapplied.
...
- 151 Sinnoh Pokédex rows, 157 encounter areas, 4,388 encounter entries, and 278 obtainable encounter profiles.
...
Pearl-specific sentinels include nighttime Misdreavus in Eterna Forest, the Armor Fossil Shieldon revival, and level-47 Palkia at Spear Pillar. Pearl and Diamond differ by 105 normalized encounter tuples in each direction.
...
`npm run catalog:audit:pearl` passes. Migration 288 imports the catalog as `pending`; migration 289 verifies exact counts, metadata, resolvable areas, and version-specific sentinels before publishing it as `verified`. Neither migration has been applied to Preview or production.
