...
- Supabase: the old migration number 261 is applied only to the isolated feature-branch Preview database; the production candidate is reconciled as migration 341 and production is unchanged
...
- Historical backfill is deliberately non-shiny. Only activity created after the Trainer Dex migration can roll a shiny.
...
The Trainer Dex migration adds:
...
Primary production-candidate migration: `supabase/341-trainer-dex-and-shiny-discoveries.sql`.
...
The feature SQL was applied to the legacy Preview under migration number 261 on August 5, 2026; that Preview history is frozen. Because production never received it, the integrated production candidate carries the identical final SQL as migration 341. Any correction after production migration 341 must use the next unused forward-only number.
...
- `src/app/globals.css`
- `src/app/sitemap.js`
- `supabase/341-trainer-dex-and-shiny-discoveries.sql`
...
5. Migration numbering is reconciled for the integrated release: Nuzlocke 261-339, including the zero-based regional Pokédex correction at 296 and single-character game-key correction at 305; tournament 340; and Trainer Dex 341. The legacy Preview's old 261 history remains frozen.
...
4. Confirm production still has migrations 013–062 prerequisites before applying migration 341 once. Do not replay the legacy Preview reconciliation bundle in production.
